...
assert(sceneLabSource.includes("Reset to scene start") && sceneLabSource.includes("setPaused?.(!paused)") && !sceneLabSource.includes("Stop and return to scene start"), "transport exposes reset and play/pause without Stop");
...
assert(sceneLabSource.includes("Reset to scene start") && sceneLabSource.includes("setPaused?.(true);this.setCurrentX(start,true)"), "Reset seeks to scene start plus pause");
