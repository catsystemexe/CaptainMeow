...
assert(sceneLabSource.includes("Reset to scene start") && sceneLabSource.includes("setPaused?.(!this.gameplayPaused())") && !sceneLabSource.includes("Stop and return to scene start"), "transport exposes reset and play/pause without Stop");
...
assert(sceneLabSource.includes("Reset to scene start") && sceneLabSource.includes("__CM?.game?.reset?.();this.setCurrentX(start,true)"), "Reset starts a fresh Level, seeks to scene start, and pauses");
