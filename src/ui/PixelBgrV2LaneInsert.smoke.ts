...
assert.match(source,/gutterRow\.append\(label,parallax,eye\)/,"lane gutters retain labels, parallax, and visibility controls");assert.doesNotMatch(source,/openV2LaneInsertMenu|cm-v2-lane-add|Add to \$\{lane\.label\}|gutterRow\.append\(label,parallax,eye,add\)/,"legacy lane add UI and wiring are absent");assert.doesNotMatch(source,/button\("Segment",\(\)=>insert\("segment"\)\)|button\("Object",\(\)=>insert\("object"\)\)/,"legacy Segment/Object chooser is absent");assert.match(source,/timeline\.style\.width=`\$\{scale\.widthPx\}px`/,"timeline layout remains intact");assert.match(source,/insertV2LaneSegment\(scene,track\.id,this\.currentX\(\),asset,selectedSegment\?\.id\)/,"P3 segment insertion helper remains in use");assert.match(source,/insertV2LaneObject\(scene,track\.id,this\.currentX\(\),asset\)/,"P3 object insertion helper remains in use");
