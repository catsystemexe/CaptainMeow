...
assert.match(source,/gutterRow\.append\(label,parallax,eye,add\)/);assert.doesNotMatch(source,/row\.appendChild\(add\)/);assert.match(source,/document\.body\.appendChild\(menu\)/);assert.match(source,/timeline\.style\.width=`\$\{scale\.widthPx\}px`/);assert.match(source,/title=`Add to \$\{lane\.label\}`/);assert.match(source,/button\("Segment"/);assert.match(source,/button\("Object"/);
