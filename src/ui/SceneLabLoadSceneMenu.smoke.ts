...
assert.match(source, /private selectScene[\s\S]*?this\.closeSceneMenu\(false\)[\s\S]*?this\.activateV2SceneForAuthoring/, "selection closes before the authoring activation transaction");
assert.match(source, /private selectScene[\s\S]*?rememberSceneLabCatalogEntry\(entry\)[\s\S]*?this\.activateV2SceneForAuthoring/, "catalog selection remembers the stable entry before authoring activation");
assert.match(source, /resolveSceneLabV2Entry\(getBackgroundSceneV2\(globalThis\)\)[\s\S]*?this\.activateV2SceneForAuthoring\(resolution\.entry\.create\(\)\)/, "DEV entry resolves through catalog and the authoring activation transaction");
assert.match(source, /private loadV2[\s\S]*?this\.activateV2SceneForAuthoring\(result\.scene\)/, "saved V2 loading uses the authoring activation transaction");
assert.match(source, /private importV2File[\s\S]*?this\.activateV2SceneForAuthoring\(result\.scene\)/, "imported V2 loading uses the authoring activation transaction");
...
assert.match(source, /dataset\.timelineMode = sceneActive && getBackgroundSceneV2\(globalThis\) \? "v2" : "disabled"/, "an active V2 scene expands and other modes collapse the center timeline");
