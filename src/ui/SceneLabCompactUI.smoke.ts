...
for (const action of ["Open scene", "Save scene", "Duplicate scene", "Close Scene Lab"]) assert(lab.includes(`this.iconButton("${action}"`), `${action} remains wired`);
assert(lab.includes('actions=el("div","cm-scene-action-row")') && lab.includes("flex-wrap:nowrap"), "scene actions share one non-wrapping compact row");
...
assert(lab.includes("Reset to scene start") && lab.includes("setCurrentX(start,true)") && lab.includes("setPaused?.(!paused)") && !lab.includes("Stop and return to scene start"), "transport remains connected to the existing runtime controls");
