...
const source=readFileSync(new URL("./PixelBgrLabUI.ts",import.meta.url),"utf8");assert.match(source,/if\(!segment&&!object\)[\s\S]*?return surface/);assert.match(source,/selectV2Event\(eventId:string[\s\S]*?v2SelectedSegmentId=""[\s\S]*?v2SelectedObjectId=""/,"Event selection clears visual selection so the Y rail is hidden");assert.match(source,/selectV2Segment\(trackId:string[\s\S]*?v2SelectedEventId=""/,"segment selection owns the contextual Y rail");assert.match(source,/selectV2Object\(trackId:string[\s\S]*?v2SelectedEventId=""/,"object selection owns the contextual Y rail");assert.match(source,/panel\.append\(this\.renderV2YRail\(\),this\.renderV2ContextualYSurface\(\)/);assert.match(source,/setPointerCapture/);assert.match(source,/releasePointerCapture/);assert.match(source,/logicH\/rect\.height/);assert.match(source,/\{offsetY:y\}/);assert.match(source,/\{y\}\)/);
