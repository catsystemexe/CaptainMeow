...
assert.match(shellRule, /grid-template-rows:\s*minmax\(0,\s*1fr\)\s*30px/, "non-V2 DEV uses only a compact status strip");
...
assert(source.includes('this.workspace.root.dataset.timelineMode = v2Scene ? "v2" : "disabled"'), "scene-aware Lab state selects shell timeline occupancy without transferring timeline ownership");
