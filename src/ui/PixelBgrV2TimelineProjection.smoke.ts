...
assert.deepEqual(segments.map(segment => [segment.id, segment.trackId, segment.startX, segment.endX, segment.enabled]), [["wide", "far-a", 1000, 3000, true], ["overlap", "far-b", 1250, 1450, false]]);
const objects = projection.lanes.find(lane => lane.id === "near")!.tracks[0].objects;
assert.deepEqual(objects.map(object => [object.id, object.trackId, object.x, object.width, object.enabled]), [["sized", "near", 600, 114.28571428571433, true], ["point", "near", 540 / .7, null, false]]);
assert.deepEqual(projection.bounds, { startX: 0, endX: 3000 });
