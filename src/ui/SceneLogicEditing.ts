...
import { validateFlowRestartLevelAction, validateFlowStartSequenceAction, validateStateDecrementAction, validateStateIncrementAction, validateStateSetAction, validateWorldStopScrollAction, type SceneLogicActionDefinition } from "../game/scene-logic/Action";
...
export type ActionDraft={category:"world";type:"stop_scroll"}|{category:"flow";type:"restart_level"}|{category:"flow";type:"start_sequence";sequenceInstanceId:string}|{category:"state";type:"set"|"increment"|"decrement";stateId:string;value:StateValue};
function actionError(action:SceneLogicActionDefinition,logic:SceneLogicDocument):string|null {const state=action.category==="state"?logic.states.find(x=>x.id===action.stateId):undefined;if(action.category==="state"&&!state)return `Missing State '${action.stateId}'.`;if(action.category==="state"&&action.type!=="set"&&state?.valueType!=="number")return "Increment/decrement require a number State.";if(action.category==="state"&&action.type==="set"&&typeof action.value!==state?.valueType)return `Set value must be ${state?.valueType}.`;if(action.category==="flow"&&action.type==="start_sequence"&&(logic.version!==2||!logic.sequenceInstances.some(item=>item.id===action.sequenceInstanceId)))return `Missing Sequence Instance '${action.sequenceInstanceId}'.`;const validation=action.category==="world"?validateWorldStopScrollAction(action):action.category==="flow"?(action.type==="start_sequence"?validateFlowStartSequenceAction(action):validateFlowRestartLevelAction(action)):action.type==="set"?validateStateSetAction(action):action.type==="increment"?validateStateIncrementAction(action):validateStateDecrementAction(action);return validation.valid?null:validation.issues[0].message;}
...
export function updateAction(scene:BackgroundSceneV2,id:string,patch:Partial<{stateId:string;value:StateValue;sequenceInstanceId:string}>):LogicEditResult {const logic=ensureSceneLogicDocument(scene);const current=logic.actions.find(x=>x.id===id);if(!current)return fail(scene,`Action '${id}' was not found.`);const next={...current,...patch,id:current.id,category:current.category,type:current.type} as SceneLogicActionDefinition;const error=actionError(next,logic);if(error)return fail(scene,error);return save(scene,{...logic,actions:logic.actions.map(action=>action.id===id?next:action)},{kind:"action",id});}
