...
assert.match(source,/panel\.append\(contextualY,this\.renderV2YRail\(\),scroll\)/,"Y controls always mount in visual order in the timeline panel");
...
assert.match(source,/readout\.textContent="Y —"/,"no visual selection has a neutral Y readout");
assert.match(source,/rail\.dataset\.yRail=hasVisualSelection\?"selection":"neutral"/,"Event-only selection cannot become the Y owner");
