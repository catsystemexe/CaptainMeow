...
export class PixelBgrLabUI {
  ...
  private renderOwned(): void {
    ...
    if (v2Scene) {
      ...
      inspector.appendChild(this.renderV2Inspector(v2Scene,projection.bounds));
      this.root.append(this.renderV2Toolbar(),inspector);
      this.syncOverlay();
      return;
    }
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    scroll.appendChild(timeline); panel.append(scroll); return panel;
  }
  ...
  private renderV2Inspector(scene:BackgroundSceneV2,bounds:{startX:number;endX:number}):HTMLElement {
    ...
    const segmentCount=scene.tracks.reduce((sum,item)=>sum+item.segments.length,0),objectCount=scene.tracks.reduce((sum,item)=>sum+item.objects.length,0);
    inspector.append(this.inspectorHeading("SCENE",scene.id),this.row("tracks",document.createTextNode(String(scene.tracks.length))),this.row("segments",document.createTextNode(String(segmentCount))),this.row("objects",document.createTextNode(String(objectCount))),this.row("environment",document.createTextNode(scene.environment.starfield?"starfield enabled":"not configured")),this.renderPreview([],bounds));
    return inspector;
  }
  private inspectorHeading(kind:string,id:string):HTMLElement {const heading=el("h4","cm-v2-inspector-title");heading.textContent=`${kind} · ${id}`;return heading;}
  private renderV2TrackInspector(track:BackgroundTrack):HTMLElement {
    const body=el("div","cm-pixel-props");
    body.append(this.row("name",document.createTextNode(track.name)),this.row("role",document.createTextNode(track.role)),this.row("mode",document.createTextNode(track.mode)),this.row("enabled",document.createTextNode(track.enabled?"yes":"no")),this.row("parallax X",document.createTextNode(String(track.parallax.x))),this.row("parallax Y",document.createTextNode(String(track.parallax.y))),this.row("zBase",document.createTextNode(String(track.zBase))),this.row("segments",document.createTextNode(String(track.segments.length))),this.row("objects",document.createTextNode(String(track.objects.length))),this.parallaxPolicyInfo());
    return body;
  }
