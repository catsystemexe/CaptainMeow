import { clearBackgroundPreviewState, getBackgroundPreviewState, getBackgroundScene, playerLevelXToPreviewScrollX, requestBackgroundMarkerRuntimeReset, resolvePlayerScreenAnchorX, setBackgroundPreviewState, setBackgroundScene, subscribeBackgroundState } from "../render/BackgroundState";
...
export class PixelBgrLabUI {
  ...
  private playerScreenAnchorX(): number { const game=(globalThis as any).__CM?.game; return resolvePlayerScreenAnchorX(Number(game?.playerEnt?.pos?.x), Number(game?.world?.scrollX), 100); }
  ...
  private currentScroll(): Point { return { x: this.previewScrollForPlayerX(this.currentX()), y: 0 }; }
