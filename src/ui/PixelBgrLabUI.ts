...
export class PixelBgrLabUI {
  ...
  private renderOwned(): void {
    ...
    if (v2Scene) {
      ...
      const contextualEvent=this.renderV2EventSurface(v2Scene);if(contextualEvent){const selectionBlock=el("div","cm-v2-left-block cm-v2-selection-block");selectionBlock.append(contextualEvent);this.root.appendChild(selectionBlock);}
      const contextualY=this.renderV2ContextualYSurface();if(contextualY){const selectionBlock=el("div","cm-v2-left-block cm-v2-selection-block");selectionBlock.append(contextualY);this.root.appendChild(selectionBlock);if(!this.v2YRailDrag)this.workspace.left.appendChild(this.renderV2YRail());}
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    timeline.style.height=`${headerHeight+(projection.lanes.length+1)*rowHeight}px`;
    ...
    const eventRow=el("div","cm-v2-lane cm-v2-event-row");eventRow.dataset.lane="events";eventRow.style.top=`${top}px`;eventRow.style.height=`${rowHeight}px`;
  ...
  private changeV2TimelineZoom(direction:-1|1):void {const candidates=direction<0?[...PIXEL_BGR_TIMELINE_ZOOM_LEVELS].reverse():PIXEL_BGR_TIMELINE_ZOOM_LEVELS;const next=candidates.find(zoom=>direction<0?zoom<this.v2TimelineZoom:zoom>this.v2TimelineZoom);if(next===undefined)return;const oldScroll=this.workspace.timeline.querySelector<HTMLElement>(".cm-v2-timeline-scroll");const cursorViewportX=oldScroll&&this.v2LastCursorPx!==null?this.v2LastCursorPx-oldScroll.scrollLeft:null;this.v2TimelineZoom=next;this.render();if(cursorViewportX!==null){const newScroll=this.workspace.timeline.querySelector<HTMLElement>(".cm-v2-timeline-scroll");if(newScroll&&this.v2LastCursorPx!==null)newScroll.scrollLeft=Math.max(0,this.v2LastCursorPx-cursorViewportX);}}
  ...
  private applyV2EventEdit(result:V2SceneEventEditResult):void {if(!result.ok){this.message=result.error;this.render();return;}this.message="";this.v2SelectedEventId=result.eventId;this.v2SelectedTrackId="";this.v2SelectedSegmentId="";this.v2SelectedObjectId="";setBackgroundSceneV2(result.scene,globalThis);}
  private beginV2EventDrag(e:PointerEvent,eventId:string,scale:TimelineScale):void {isolateTimelinePointerEvent(e);this.endV2EventDrag();const baseline=getBackgroundSceneV2(globalThis);if(!baseline)return;this.v2SelectedEventId=eventId;this.v2SelectedTrackId="";this.v2SelectedSegmentId="";this.v2SelectedObjectId="";const captureTarget=e.currentTarget instanceof HTMLElement?e.currentTarget:null;captureTarget?.setPointerCapture?.(e.pointerId);this.v2EventDrag={pointerId:e.pointerId,eventId,startClientX:e.clientX,scale,baseline,captureTarget,active:true};window.addEventListener("pointermove",this.onV2EventPointerMove);window.addEventListener("pointerup",this.onV2EventPointerUp);window.addEventListener("pointercancel",this.onV2EventPointerUp);}
  ...
  private selectV2Track(trackId:string,render=true):void {this.v2SelectedEventId="";this.v2SelectedTrackId=trackId;this.v2SelectedSegmentId="";this.v2SelectedObjectId="";this.v2PlacementTarget=null;if(render)this.render();}
  private selectV2Segment(trackId:string,segmentId:string,render=true):void {this.v2SelectedEventId="";this.v2SelectedTrackId=trackId;this.v2SelectedSegmentId=segmentId;this.v2SelectedObjectId="";if(render)this.render();}
  private selectV2Object(trackId:string,objectId:string,render=true):void {this.v2SelectedEventId="";this.v2SelectedTrackId=trackId;this.v2SelectedSegmentId="";this.v2SelectedObjectId=objectId;if(render)this.render();}
  private renderV2EventSurface(scene:BackgroundSceneV2):HTMLElement|null {const event=scene.events?.find(item=>item.id===this.v2SelectedEventId);if(!event)return null;const surface=el("section","cm-v2-selected-y");const title=el("div","cm-v2-selected-y-title");title.textContent=`Selected · ${event.type==="signal"?event.name:"level-end"}`;surface.append(title,this.row("X",this.numericStepper({value:event.worldX,step:16,min:0,onCommit:worldX=>this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{worldX}))})),this.row("type",document.createTextNode(event.type)));if(event.type==="signal")surface.appendChild(this.row("name",text(event.name,name=>this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{name})))));surface.append(this.row("enabled",check(event.enabled,enabled=>this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{enabled})))),button("delete",()=>this.applyV2EventEdit(deleteV2SceneEvent(scene,event.id))));return surface;}
