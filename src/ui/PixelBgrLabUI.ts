...
export class PixelBgrLabUI {
  ...
  private v2Rename: { kind:"segment"|"object"|"event"; trackId?:string; id:string } | null = null;
  ...
  private sceneContentsVisualChild(kind:"segment"|"object",trackId:string,id:string,enabled:boolean,name?:string,locked?:boolean):HTMLElement {const selected=this.v2SelectedTrackId===trackId&&(kind==="segment"?this.v2SelectedSegmentId===id:this.v2SelectedObjectId===id);const row=el("div",`cm-scene-tree-child${selected?" sel":""}`);const toggle=()=>{const scene=getBackgroundSceneV2(globalThis);if(!scene)return;if(kind==="segment"){const result=updateV2Segment(scene,trackId,id,{enabled:!enabled});if(result.ok)setBackgroundSceneV2(result.scene,globalThis);}else{const result=updateV2Object(scene,trackId,id,{enabled:!enabled});if(result.ok)setBackgroundSceneV2(result.scene,globalThis);}};const eye=this.iconButton(`${enabled?"Hide":"Show"} ${kind} ${id}`,enabled?Eye:EyeOff,enabled?"Eye":"EyeOff",toggle);eye.className="cm-scene-tree-action";eye.onclick=e=>{e.stopPropagation();toggle();};const label=name?.trim()||id;const select=button(label,()=>kind==="segment"?this.selectV2Segment(trackId,id):this.selectV2Object(trackId,id));select.className="cm-scene-tree-child-label";const editing=this.v2Rename?.kind===kind&&this.v2Rename.trackId===trackId&&this.v2Rename.id===id;if(editing)row.append(eye,this.v2RenameInput(label,value=>{const scene=getBackgroundSceneV2(globalThis);if(!scene)return;const patch={name:value||undefined};kind==="segment"?this.applyV2Edit(updateV2Segment(scene,trackId,id,patch)):this.applyV2ObjectEdit(updateV2Object(scene,trackId,id,patch));}),document.createTextNode(locked?`${trackId} · locked`:trackId));else row.append(eye,select,document.createTextNode(locked?`${trackId} · locked`:trackId));row.oncontextmenu=e=>this.openV2VisualContextMenu(e,trackId,kind,id);return row;}
  private sceneContentsEventChild(scene:BackgroundSceneV2,event:NonNullable<BackgroundSceneV2["events"]>[number],index:number):HTMLElement {const name=event.name?.trim()||event.id;const row=el("div",`cm-scene-tree-child cm-scene-tree-event${event.id===this.v2SelectedEventId?" sel":""}`);const toggle=()=>{const result=updateV2SceneEvent(scene,event.id,{enabled:!event.enabled});if(result.ok)setBackgroundSceneV2(result.scene,globalThis);};const power=this.iconButton(`${event.enabled?"Disable":"Enable"} Event ${name}`,Power,"Power",toggle);power.className="cm-scene-tree-action";power.dataset.action="activation";power.dataset.state=event.enabled?"all":"none";power.onclick=e=>{e.stopPropagation();toggle();};const ordinal=el("span","cm-scene-tree-ordinal");ordinal.textContent=`${index+1}.`;const label=button(name,()=>this.selectV2Event(event.id));label.className="cm-scene-tree-child-label";const type=el("span","cm-v2-logic-type");type.textContent=event.locked?`${event.type} · locked`:event.type;row.append(power,ordinal,label,type);if(this.v2Rename?.kind==="event"&&this.v2Rename.id===event.id)row.replaceChild(this.v2RenameInput(name,value=>this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{name:value||undefined}))),label);row.oncontextmenu=e=>this.openV2EventContextMenu(e,event.id);return row;}
  ...
  private openV2EntityContextMenu(e:MouseEvent,label:string,actions:readonly {name:string;icon:IconNode;iconName:string;action:()=>void;disabled?:boolean;destructive?:boolean;separator?:boolean}[]):void {e.preventDefault();e.stopPropagation();this.closeEntityContextMenu?.();const menu=el("div","cm-v2-entity-menu");menu.setAttribute("role","menu");menu.setAttribute("aria-label",`${label} actions`);const close=()=>{menu.remove();document.removeEventListener("pointerdown",outside);document.removeEventListener("keydown",keydown);if(this.closeEntityContextMenu===close)this.closeEntityContextMenu=null;};const outside=(event:PointerEvent)=>{if(event.target instanceof Node&&!menu.contains(event.target))close();};const keydown=(event:KeyboardEvent)=>{if(event.key==="Escape"){event.preventDefault();close();}};for(const action of actions){if(action.separator)menu.appendChild(el("hr"));const item=this.iconButton(action.name,action.icon,action.iconName,()=>{close();action.action();},action.disabled);item.setAttribute("role","menuitem");item.append(document.createTextNode(action.name));if(action.destructive)item.classList.add("destructive");menu.appendChild(item);}menu.style.left=`${e.clientX}px`;menu.style.top=`${e.clientY}px`;document.body.appendChild(menu);this.closeEntityContextMenu=close;document.addEventListener("pointerdown",outside);document.addEventListener("keydown",keydown);menu.querySelector("button")?.focus();}
  private openV2VisualContextMenu(e:MouseEvent,trackId:string,kind:"segment"|"object",itemId:string):void {isolateTimelinePointerEvent(e);kind==="segment"?this.selectV2Segment(trackId,itemId,false):this.selectV2Object(trackId,itemId,false);const scene=getBackgroundSceneV2(globalThis);const track=scene&&findV2Track(scene,trackId);const item=kind==="segment"?track?.segments.find(value=>value.id===itemId):track?.objects.find(value=>value.id===itemId);if(!scene||!track||!item)return;const locked=item.locked===true;const readonly=kind==="segment"&&!canAuthorV2Segments(track);const disabled=locked||readonly;const edit=(patch:V2SegmentPatch|V2ObjectPatch)=>kind==="segment"?this.applyV2Edit(updateV2Segment(scene,trackId,itemId,patch as V2SegmentPatch)):this.applyV2ObjectEdit(updateV2Object(scene,trackId,itemId,patch as V2ObjectPatch));this.openV2EntityContextMenu(e,kind,[{name:locked?"Unlock":"Lock",icon:locked?Unlock:Lock,iconName:locked?"Unlock":"Lock",disabled:readonly,action:()=>edit({locked:!locked})},{name:"Duplicate",icon:Copy,iconName:"Copy",disabled,separator:true,action:()=>kind==="segment"?this.applyV2Edit(duplicateV2Segment(scene,trackId,itemId)):this.applyV2ObjectEdit(duplicateV2Object(scene,trackId,itemId))},{name:"Flip Horizontal",icon:FlipHorizontal2,iconName:"FlipHorizontal2",disabled,action:()=>edit({flipX:!item.flipX})},{name:"Flip Vertical",icon:FlipVertical2,iconName:"FlipVertical2",disabled,action:()=>edit({flipY:!item.flipY})},{name:"Rename",icon:Pencil,iconName:"Pencil",disabled,action:()=>{this.v2Rename={kind,trackId,id:itemId};this.render();}},{name:"Delete",icon:Trash2,iconName:"Trash2",disabled,destructive:true,separator:true,action:()=>kind==="segment"?this.applyV2Edit(deleteV2Segment(scene,trackId,itemId)):this.applyV2ObjectEdit(deleteV2Object(scene,trackId,itemId))}]);}
  private openV2EventContextMenu(e:MouseEvent,eventId:string):void {isolateTimelinePointerEvent(e);this.endV2EventDrag();this.selectV2Event(eventId,false);const scene=getBackgroundSceneV2(globalThis);const event=scene?.events?.find(item=>item.id===eventId);if(!scene||!event)return;const locked=event.locked===true;this.openV2EntityContextMenu(e,"Event",[{name:locked?"Unlock":"Lock",icon:locked?Unlock:Lock,iconName:locked?"Unlock":"Lock",action:()=>this.applyV2EventEdit(updateV2SceneEvent(scene,eventId,{locked:!locked}))},{name:"Duplicate",icon:Copy,iconName:"Copy",disabled:locked||event.type==="level-end",separator:true,action:()=>this.applyV2EventEdit(duplicateV2SceneEvent(scene,eventId))},{name:"Rename",icon:Pencil,iconName:"Pencil",disabled:locked,action:()=>{this.v2Rename={kind:"event",id:eventId};this.render();}},{name:"Delete",icon:Trash2,iconName:"Trash2",disabled:locked,destructive:true,separator:true,action:()=>this.applyV2EventEdit(deleteV2SceneEvent(scene,eventId))}]);}
