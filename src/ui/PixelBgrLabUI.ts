...
import { createMarker as createLogicMarker, createRange as createLogicRange, createZone as createLogicZone, deleteSpace, findSpace, updateMarker as updateLogicMarker, updateRange as updateLogicRange, updateZone as updateLogicZone, type SpaceEditResult, type V2SpaceSelection } from "./SceneLogicSpaceEditing";
import { dragRange, moveZone, zoneToCanvasRect, type SpaceTimelineDragMode } from "./SceneLogicSpaceProjection";
import { formatWorldX } from "./SceneLabWorldXFormatting";
import { bindEventAction, bindTriggerEvent, createAction, createSceneEvent as createLogicEvent, createSpaceTrigger, createStateReference, createStateTrigger, createTimeTrigger, deleteAction as deleteLogicAction, deleteSceneEvent as deleteLogicEvent, deleteStateReference, deleteTrigger, STATE_REFERENCE_PRESETS, unbindEventAction, unbindTriggerEvent, updateSceneEvent as updateLogicEvent, updateTrigger, type LogicEditResult, type LogicSelection } from "./SceneLogicEditing";
...
type V2RenameTarget = { kind:"event"; id:string; surface:"timeline"|"tree" };
type V2RightContext = "scene-asset" | "space" | "logic";
...
export class PixelBgrLabUI {
  ...
  private v2SelectedSpace: V2SpaceSelection | null = null;
  private v2SelectedLogic: LogicSelection | null = null;
  private v2RightContext: V2RightContext = "scene-asset";
  ...
  private v2SpaceDrag: { pointerId:number; selection:V2SpaceSelection; mode:SpaceTimelineDragMode; startClientX:number; scale:TimelineScale; baseline:BackgroundSceneV2; captureTarget:HTMLElement|null } | null = null;
  private zoneDrag: { pointerId:number; zoneId:string; anchor:Point; baseline:BackgroundSceneV2 } | null = null;
  ...
  private readonly sceneContentsExpanded = new Set<string>(["SEG", "OBJ", "EVE", "LOGIC", "SPACE"]);
  private closeInsertMenu: (() => void) | null = null;
  ...
  private renderOwned(): void {
    ...
    if (v2Scene) {
      if(this.v2RightContext==="space"&&(!this.v2SelectedSpace||!findSpace(v2Scene,this.v2SelectedSpace)))this.v2RightContext="scene-asset";
      if (this.getActiveDevLab() === "scene") this.workspace.right.appendChild(this.v2RightContext==="space" ? this.renderSpaceInspector(v2Scene) : this.v2RightContext==="logic" ? this.renderLogicInspector(v2Scene) : this.renderSceneAssetContext());
  ...
  private renderV2SceneContents(scene: BackgroundSceneV2): HTMLElement {
    ...
    tree.append(
      ...
      this.sceneContentsLogicRow(scene),
      this.sceneContentsSpaceRow(scene,spaceCount)
    );
    if(this.message){const message=el("div","cm-pixel-msg");message.textContent=this.message;tree.append(message);}return tree;
  }
  private sceneContentsLogicRow(scene:BackgroundSceneV2):HTMLElement {const logic=scene.sceneLogic;const count=(logic?.triggers.length??0)+(logic?.events.length??0)+(logic?.actions.length??0);const section=el("div","cm-scene-tree-category cm-scene-logic-category");section.dataset.category="LOGIC";const row=el("div","cm-scene-tree-row");const add=button("+",()=>this.openLogicInsertMenu(add));add.className="cm-scene-tree-action cm-logic-add";add.setAttribute("aria-label","Add Scene Logic");const open=this.sceneContentsExpanded.has("LOGIC");const label=button("LOGIC",()=>{open?this.sceneContentsExpanded.delete("LOGIC"):this.sceneContentsExpanded.add("LOGIC");this.render();});label.className="cm-scene-tree-label";const meta=el("span","cm-scene-tree-meta");meta.textContent=String(count);row.append(add,label,meta);section.append(row);if(open&&logic){const list=el("div","cm-scene-tree-children");for(const [kind,items] of [["trigger",logic.triggers],["event",logic.events],["action",logic.actions]] as const){const heading=el("div","cm-v2-logic-heading");heading.textContent=kind.toUpperCase();list.append(heading);for(const item of items){const selection={kind,id:item.id} as LogicSelection;const child=el("div",`cm-scene-tree-child cm-scene-tree-logic${this.v2SelectedLogic?.kind===kind&&this.v2SelectedLogic.id===item.id?" sel":""}`);const subtype=el("span","cm-v2-logic-type");subtype.textContent=kind[0].toUpperCase();const choose=button(item.id,()=>this.selectLogic(selection));choose.className="cm-scene-tree-child-label";child.append(subtype,choose);list.append(child);}}const states=el("div","cm-v2-logic-heading");states.textContent="STATE REFERENCES";list.append(states);for(const state of logic.states){const child=el("div","cm-scene-tree-child");child.append(document.createTextNode("S"),document.createTextNode(`${state.id} · ${state.address}`),button("×",()=>this.applyLogicEdit(deleteStateReference(scene,state.id))));list.append(child);}section.append(list);}return section;}
  private openLogicInsertMenu(trigger:HTMLElement):void {this.closeInsertMenu?.();const scene=getBackgroundSceneV2(globalThis);if(!scene)return;const logic=scene.sceneLogic;const menu=el("div","cm-v2-insert-menu cm-logic-insert-menu");menu.setAttribute("role","menu");menu.setAttribute("aria-label","Add Scene Logic");const close=()=>{menu.remove();if(this.closeInsertMenu===close)this.closeInsertMenu=null;};const apply=(result:LogicEditResult)=>{close();this.applyLogicEdit(result);};const firstMarker=logic?.spaces.markers[0],firstRange=logic?.spaces.ranges[0],firstZone=logic?.spaces.zones[0],firstState=logic?.states[0],firstNumber=logic?.states.find(x=>x.valueType==="number");menu.append(button("Trigger · Marker cross",()=>{if(firstMarker)apply(createSpaceTrigger(scene,"marker",firstMarker.id,"cross"))}),button("Trigger · Range enter",()=>{if(firstRange)apply(createSpaceTrigger(scene,"range",firstRange.id,"enter"))}),button("Trigger · Zone enter",()=>{if(firstZone)apply(createSpaceTrigger(scene,"zone",firstZone.id,"enter"))}),button("Trigger · Time at",()=>apply(createTimeTrigger(scene,"at"))),button("Trigger · Time after",()=>apply(createTimeTrigger(scene,"after"))),button("Trigger · State comparison",()=>{if(firstState)apply(createStateTrigger(scene,firstState.id))}),button("Event · Scene",()=>{const type=prompt("Semantic Event type", "scene_started");if(type!==null)apply(createLogicEvent(scene,type));}),button("Action · World.stop_scroll",()=>apply(createAction(scene,{category:"world",type:"stop_scroll"}))),button("Action · State.set",()=>{if(firstState)apply(createAction(scene,{category:"state",type:"set",stateId:firstState.id,value:firstState.valueType==="boolean"?false:firstState.valueType==="number"?0:""}))}),button("Action · State.increment",()=>{if(firstNumber)apply(createAction(scene,{category:"state",type:"increment",stateId:firstNumber.id,value:1}))}),button("Action · State.decrement",()=>{if(firstNumber)apply(createAction(scene,{category:"state",type:"decrement",stateId:firstNumber.id,value:1}))}),button("Action · Flow.restart_level",()=>apply(createAction(scene,{category:"flow",type:"restart_level"}))));for(const preset of STATE_REFERENCE_PRESETS.filter(p=>!logic?.states.some(s=>s.id===p.id)))menu.append(button(`State ref · ${preset.address}`,()=>apply(createStateReference(scene,preset.id))));for(const b of Array.from(menu.querySelectorAll("button"))){if((b.textContent?.includes("Marker")&&!firstMarker)||(b.textContent?.includes("Range")&&!firstRange)||(b.textContent?.includes("Zone")&&!firstZone)||(b.textContent?.includes("State")&&!firstState)||(b.textContent?.includes("increment")&&!firstNumber)||(b.textContent?.includes("decrement")&&!firstNumber)){b.disabled=true;b.title="Create a compatible Space or State reference first.";}}const rect=trigger.getBoundingClientRect();menu.style.left=`${rect.left}px`;menu.style.top=`${rect.bottom+2}px`;document.body.append(menu);this.closeInsertMenu=close;}
  private sceneContentsSpaceRow(scene:BackgroundSceneV2,count:number):HTMLElement {
    const section=el("div","cm-scene-tree-category cm-scene-space-category");section.dataset.category="SPACE";const row=el("div","cm-scene-tree-row");
    const add=button("+",()=>this.openSpaceInsertMenu(add));add.className="cm-scene-tree-action cm-space-add";add.setAttribute("aria-label","Add Space");
    const open=this.sceneContentsExpanded.has("SPACE");const label=button("SPACE",()=>{open?this.sceneContentsExpanded.delete("SPACE"):this.sceneContentsExpanded.add("SPACE");this.render();});label.className="cm-scene-tree-label";label.dataset.accordionAction="true";
    const meta=el("span","cm-scene-tree-meta");meta.textContent=String(count);row.append(add,label,meta);section.append(row);
    if(count&&(open||count===1)){const list=el("div","cm-scene-tree-children");const spaces=scene.sceneLogic!.spaces;list.append(...spaces.markers.map(item=>this.sceneContentsSpaceChild({kind:"marker",id:item.id})),...spaces.ranges.map(item=>this.sceneContentsSpaceChild({kind:"range",id:item.id})),...spaces.zones.map(item=>this.sceneContentsSpaceChild({kind:"zone",id:item.id})));section.append(list);}return section;
  }
  private sceneContentsSpaceChild(selection:V2SpaceSelection):HTMLElement {const selected=this.v2SelectedSpace?.kind===selection.kind&&this.v2SelectedSpace.id===selection.id;const row=el("div",`cm-scene-tree-child cm-scene-tree-space${selected?" sel":""}`);const subtype=el("span","cm-v2-logic-type");subtype.textContent=selection.kind[0].toUpperCase();const label=button(selection.id,()=>this.selectV2Space(selection));label.className="cm-scene-tree-child-label";row.append(subtype,label);return row;}
  private openSpaceInsertMenu(trigger:HTMLElement):void {this.closeInsertMenu?.();const menu=el("div","cm-v2-insert-menu cm-space-insert-menu");menu.setAttribute("role","menu");menu.setAttribute("aria-label","Add Space");const close=()=>{menu.remove();if(this.closeInsertMenu===close)this.closeInsertMenu=null;};const insert=(kind:V2SpaceSelection["kind"])=>{close();const scene=getBackgroundSceneV2(globalThis);if(!scene)return;const result=kind==="marker"?createLogicMarker(scene,this.currentX()):kind==="range"?createLogicRange(scene,this.currentX()):createLogicZone(scene,this.currentX(),this.logicH);this.applySpaceEdit(result);};menu.append(button("Marker",()=>insert("marker")),button("Range",()=>insert("range")),button("Zone",()=>insert("zone")));const rect=trigger.getBoundingClientRect();menu.style.left=`${rect.left}px`;menu.style.top=`${rect.bottom+2}px`;document.body.appendChild(menu);this.closeInsertMenu=close;}
  ...
  private openSceneAssetInsertMenu(event:MouseEvent,item:SceneAssetContextItem):void {
    event.stopPropagation();this.closeInsertMenu?.();
    const menu=el("div","cm-v2-insert-menu cm-scene-asset-insert-menu");menu.setAttribute("role","menu");menu.setAttribute("aria-label",`Insert ${item.id}`);
    const anchor=event.currentTarget instanceof HTMLElement?event.currentTarget:null;
    const close=()=>{menu.remove();document.removeEventListener("pointerdown",outside);document.removeEventListener("keydown",keydown);if(this.closeInsertMenu===close)this.closeInsertMenu=null;};
    ...
    const heading=el("div","cm-scene-asset-insert-heading");heading.textContent=item.id;menu.appendChild(heading);
    ...
    document.body.appendChild(menu);const margin=6;const rect=menu.getBoundingClientRect();menu.style.left=`${Math.min(Math.max(margin,event.clientX),Math.max(margin,window.innerWidth-rect.width-margin))}px`;menu.style.top=`${Math.min(Math.max(margin,event.clientY),Math.max(margin,window.innerHeight-rect.height-margin))}px`;this.closeInsertMenu=close;document.addEventListener("pointerdown",outside);document.addEventListener("keydown",keydown);menu.querySelector<HTMLButtonElement>("button")?.focus();
  }
  ...
  private sceneContentsVisualChild(kind:"segment"|"object",trackId:string,id:string,assetId:string,enabled:boolean):HTMLElement {const selected=this.v2SelectedTrackId===trackId&&(kind==="segment"?this.v2SelectedSegmentId===id:this.v2SelectedObjectId===id);const row=el("div",`cm-scene-tree-child${selected?" sel":""}`);const toggle=()=>{const scene=getBackgroundSceneV2(globalThis);if(!scene)return;if(kind==="segment"){const result=updateV2Segment(scene,trackId,id,{enabled:!enabled});if(result.ok)setBackgroundSceneV2(result.scene,globalThis);}else{const result=updateV2Object(scene,trackId,id,{enabled:!enabled});if(result.ok)setBackgroundSceneV2(result.scene,globalThis);}};const eye=this.iconButton(`${enabled?"Hide":"Show"} ${kind} ${id}`,enabled?Eye:EyeOff,enabled?"Eye":"EyeOff",toggle);eye.className="cm-scene-tree-action";eye.onclick=e=>{e.stopPropagation();toggle();};const select=button(assetId,()=>kind==="segment"?this.selectV2Segment(trackId,id):this.selectV2Object(trackId,id));select.className="cm-scene-tree-child-label";select.title=`${assetId}\ninstance: ${id}`;row.append(eye,select);row.oncontextmenu=e=>this.openV2VisualContextMenu(e,trackId,kind,id,"tree");return row;}
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    for(const lane of projection.lanes){
      ...
      gutterRow.append(label,parallax,eye);gutter.appendChild(gutterRow);
    }
    ...
    const eventsAdd=button("+",()=>this.openV2EventInsertMenu(eventsAdd));eventsAdd.className="cm-v2-event-add";eventsAdd.title="Add scene event";eventsAdd.setAttribute("aria-label","Add scene event");eventsAdd.onpointerdown=isolateTimelinePointerEvent;
    ...
    const spaceRowHeight=27;
    const headerHeight=20;
    timeline.style.height=`${headerHeight+projection.lanes.length*rowHeight+eventRowHeight+spaceRowHeight}px`;
    ...
    projection.events.forEach((event,index)=>{const selected=this.v2SelectedEventId===event.id;const marker=el("span",`cm-v2-event ${event.type}${event.enabled?"":" disabled"}${selected?" sel":""}`);marker.style.left=`${worldToTimelinePx(event.worldX,scale)}px`;const renaming=this.v2Rename?.surface==="timeline"&&this.v2Rename.kind==="event"&&this.v2Rename.id===event.id;if(renaming)marker.append(this.v2TimelineRenameInput(event.label,value=>{const scene=getBackgroundSceneV2(globalThis);if(scene)this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{name:value||undefined}));}));else marker.textContent=String(index+1);marker.title=event.label;marker.setAttribute("aria-label",`Event ${index+1}: ${event.label}, ${event.type}, X ${formatWorldX(event.worldX)}`);marker.onpointerdown=e=>{if(e.button===2){isolateTimelinePointerEvent(e);this.selectV2Event(event.id,false);return;}this.beginV2EventDrag(e,event.id,scale);};marker.oncontextmenu=e=>this.openV2EventContextMenu(e,event.id,"timeline");eventRow.appendChild(marker);});timeline.appendChild(eventRow);top+=eventRowHeight;
    const spaceRow=el("div","cm-v2-lane cm-space-lane");spaceRow.dataset.lane="spaces";spaceRow.style.top=`${top}px`;spaceRow.style.height=`${spaceRowHeight}px`;for(const item of logicSpaces?.markers??[]){const selection={kind:"marker" as const,id:item.id};const marker=el("span",`cm-space-marker${this.v2SelectedSpace?.kind==="marker"&&this.v2SelectedSpace.id===item.id?" sel":""}`);marker.style.left=`${worldToTimelinePx(item.position,scale)}px`;marker.title=`Marker ${item.id} @ ${formatWorldX(item.position)}`;marker.onpointerdown=e=>this.beginSpaceTimelineDrag(e,selection,"move",scale);spaceRow.append(marker);}for(const item of logicSpaces?.ranges??[]){const selection={kind:"range" as const,id:item.id};const block=el("span",`cm-space-range${this.v2SelectedSpace?.kind==="range"&&this.v2SelectedSpace.id===item.id?" sel":""}`);block.style.left=`${worldToTimelinePx(item.start,scale)}px`;block.style.width=`${Math.max(3,worldToTimelinePx(item.end,scale)-worldToTimelinePx(item.start,scale))}px`;block.title=`Range ${item.id} ${formatWorldX(item.start)}..${formatWorldX(item.end)}`;block.onpointerdown=e=>this.beginSpaceTimelineDrag(e,selection,"move",scale);for(const side of ["left","right"] as const){const handle=el("span",`cm-space-range-handle ${side}`);handle.onpointerdown=e=>this.beginSpaceTimelineDrag(e,selection,side==="left"?"resize-left":"resize-right",scale);block.append(handle);}spaceRow.append(block);}timeline.appendChild(spaceRow);
  ...
  private openV2EventInsertMenu(trigger:HTMLElement):void {this.closeInsertMenu?.();const menu=el("div","cm-v2-insert-menu");menu.setAttribute("role","menu");menu.setAttribute("aria-label","Add scene event");const close=()=>{menu.remove();if(this.closeInsertMenu===close)this.closeInsertMenu=null;};const insert=(type:"signal"|"level-end")=>{close();const scene=getBackgroundSceneV2(globalThis);if(scene)this.applyV2EventEdit(createV2SceneEvent(scene,this.currentX(),{type}));};menu.append(button("Signal",()=>insert("signal")),button("Level End",()=>insert("level-end")));const rect=trigger.getBoundingClientRect();menu.style.left=`${rect.left}px`;menu.style.top=`${rect.bottom+2}px`;document.body.appendChild(menu);this.closeInsertMenu=close;}
  ...
  private beginSpaceTimelineDrag(e:PointerEvent,selection:V2SpaceSelection,mode:SpaceTimelineDragMode,scale:TimelineScale):void {isolateTimelinePointerEvent(e);this.endSpaceTimelineDrag();const baseline=getBackgroundSceneV2(globalThis);if(!baseline)return;this.selectV2Space(selection,false);const captureTarget=e.currentTarget instanceof HTMLElement?e.currentTarget:null;captureTarget?.setPointerCapture?.(e.pointerId);this.v2SpaceDrag={pointerId:e.pointerId,selection,mode,startClientX:e.clientX,scale,baseline,captureTarget};window.addEventListener("pointermove",this.onSpaceTimelinePointerMove);window.addEventListener("pointerup",this.onSpaceTimelinePointerUp);window.addEventListener("pointercancel",this.onSpaceTimelinePointerUp);}
  private onSpaceTimelinePointerMove=(e:PointerEvent):void=>{const drag=this.v2SpaceDrag;if(!drag||drag.pointerId!==e.pointerId)return;isolateTimelinePointerEvent(e);const source=findSpace(drag.baseline,drag.selection);if(!source)return;const delta=timelinePointerDeltaWorld(drag.startClientX,e.clientX,drag.scale);if(drag.selection.kind==="marker"&&"position" in source)this.applySpaceEdit(updateLogicMarker(drag.baseline,source.id,{position:source.position+delta}));else if(drag.selection.kind==="range"&&"start" in source)this.applySpaceEdit(updateLogicRange(drag.baseline,source.id,dragRange(source,drag.mode,delta)));};
  private onSpaceTimelinePointerUp=(e:PointerEvent):void=>{if(this.v2SpaceDrag?.pointerId!==e.pointerId)return;isolateTimelinePointerEvent(e);this.endSpaceTimelineDrag();this.render();};
  private endSpaceTimelineDrag():void {const drag=this.v2SpaceDrag;if(!drag)return;drag.captureTarget?.releasePointerCapture?.(drag.pointerId);this.v2SpaceDrag=null;window.removeEventListener("pointermove",this.onSpaceTimelinePointerMove);window.removeEventListener("pointerup",this.onSpaceTimelinePointerUp);window.removeEventListener("pointercancel",this.onSpaceTimelinePointerUp);}

  private selectV2Event(eventId:string,render=true):void {this.v2SelectedEventId=eventId;this.v2RightContext="scene-asset";if(render)this.render();}
  private selectV2Track(trackId:string,render=true):void {this.v2SelectedTrackId=trackId;this.v2SelectedSegmentId="";this.v2SelectedObjectId="";this.v2PlacementTarget=null;this.v2RightContext="scene-asset";if(render)this.render();}
  private selectV2Segment(trackId:string,segmentId:string,render=true):void {this.v2SelectedTrackId=trackId;this.v2SelectedSegmentId=segmentId;this.v2SelectedObjectId="";this.v2RightContext="scene-asset";const scene=getBackgroundSceneV2(globalThis);const segment=scene?findV2Segment(scene,trackId,segmentId):null;if(segment)this.v2SelectedAssetId=syncV2PickerAssetId(segment.asset.id);if(render)this.render();}
  private selectV2Object(trackId:string,objectId:string,render=true):void {this.v2SelectedTrackId=trackId;this.v2SelectedSegmentId="";this.v2SelectedObjectId=objectId;this.v2RightContext="scene-asset";const scene=getBackgroundSceneV2(globalThis);const object=scene?findV2Object(scene,trackId,objectId):null;if(object)this.v2SelectedAssetId=syncV2PickerAssetId(object.asset.id);if(render)this.render();}
  ...
  private selectLogic(selection:LogicSelection,render=true):void {this.v2SelectedLogic=selection;this.v2RightContext="logic";if(render)this.render();}
  private applyLogicEdit(result:LogicEditResult):void {if(!result.ok){this.message=result.error;this.render();return;}this.message="";if(result.selection)this.v2SelectedLogic=result.selection;setBackgroundSceneV2(result.scene,globalThis);}
  private renderLogicInspector(scene:BackgroundSceneV2):HTMLElement {const panel=el("section","cm-v2-inspector cm-logic-inspector");panel.dataset.inspector="logic";const selection=this.v2SelectedLogic,logic=scene.sceneLogic;if(!selection||!logic){panel.textContent="Select canonical Scene Logic";return panel;}const item=selection.kind==="trigger"?logic.triggers.find(x=>x.id===selection.id):selection.kind==="event"?logic.events.find(x=>x.id===selection.id):logic.actions.find(x=>x.id===selection.id);if(!item){panel.textContent="Select canonical Scene Logic";return panel;}panel.append(this.inspectorHeading(selection.kind.toUpperCase(),selection.id),this.row("ID",document.createTextNode(selection.id)));
    if(selection.kind==="trigger"){const trigger=item as typeof logic.triggers[number];panel.append(this.row("TYPE / KIND",document.createTextNode(trigger.kind)),this.row("ENABLED",check(trigger.enabled,v=>this.applyLogicEdit(updateTrigger(scene,trigger.id,{enabled:v})))),this.row("MODE",this.select(trigger.mode,["once","repeat"],v=>this.applyLogicEdit(updateTrigger(scene,trigger.id,{mode:v as "once"|"repeat"})))));if(trigger.kind==="time")panel.append(this.row("RELATION",this.select(trigger.relation,["at","after"],v=>this.applyLogicEdit(updateTrigger(scene,trigger.id,{relation:v as "at"|"after"})))),this.row("TIME",num(trigger.timeSec,.1,v=>this.applyLogicEdit(updateTrigger(scene,trigger.id,{timeSec:v})))));else if(trigger.kind==="state")panel.append(this.row("STATE",document.createTextNode(trigger.stateId)),this.row("RELATION",document.createTextNode(trigger.relation)),this.row("VALUE",document.createTextNode(String(trigger.value))));else {const ref="markerId" in trigger?trigger.markerId:"rangeId" in trigger?trigger.rangeId:trigger.zoneId;panel.append(this.row("SPACE",document.createTextNode(ref)),this.row("RELATION",document.createTextNode(trigger.relation)));}panel.append(document.createTextNode("EVENTS"));for(const binding of logic.triggerEventBindings.filter(x=>x.triggerId===trigger.id))panel.append(this.row(binding.eventId,button("remove binding",()=>this.applyLogicEdit(unbindTriggerEvent(scene,trigger.id,binding.eventId)))));const available=logic.events.filter(e=>!logic.triggerEventBindings.some(b=>b.triggerId===trigger.id&&b.eventId===e.id));if(available[0])panel.append(button("+ bind Event",()=>this.applyLogicEdit(bindTriggerEvent(scene,trigger.id,available[0].id))));panel.append(button("Delete Trigger",()=>this.applyLogicEdit(deleteTrigger(scene,trigger.id))));}
    else if(selection.kind==="event"){const event=item as typeof logic.events[number];panel.append(this.row("CATEGORY",document.createTextNode("Scene")),this.row("TYPE",text(event.type,v=>this.applyLogicEdit(updateLogicEvent(scene,event.id,v)))));panel.append(document.createTextNode("SOURCES"));for(const b of logic.triggerEventBindings.filter(x=>x.eventId===event.id))panel.append(document.createTextNode(b.triggerId));panel.append(document.createTextNode("ACTIONS"));for(const b of logic.eventActionBindings.filter(x=>x.eventId===event.id))panel.append(this.row(b.actionId,button("remove binding",()=>this.applyLogicEdit(unbindEventAction(scene,event.id,b.actionId)))));const available=logic.actions.filter(a=>!logic.eventActionBindings.some(b=>b.eventId===event.id&&b.actionId===a.id));if(available[0])panel.append(button("+ bind Action",()=>this.applyLogicEdit(bindEventAction(scene,event.id,available[0].id))));panel.append(button("Delete Event",()=>this.applyLogicEdit(deleteLogicEvent(scene,event.id))));}
    else {const action=item as typeof logic.actions[number];panel.append(this.row("CATEGORY",document.createTextNode(action.category)),this.row("TYPE",document.createTextNode(action.type)));if(action.category==="state")panel.append(this.row("STATE",document.createTextNode(action.stateId)),this.row("VALUE",document.createTextNode(String(action.value))));panel.append(document.createTextNode("USED BY EVENTS"));for(const b of logic.eventActionBindings.filter(x=>x.actionId===action.id))panel.append(document.createTextNode(b.eventId));panel.append(button("Delete Action",()=>this.applyLogicEdit(deleteLogicAction(scene,action.id))));}if(this.message){const message=el("div","cm-pixel-msg");message.textContent=this.message;panel.append(message);}return panel;}
  private selectV2Space(selection:V2SpaceSelection,render=true):void {this.v2SelectedSpace=selection;this.v2RightContext="space";if(render){this.render();this.syncOverlay();}}
  private applySpaceEdit(result:SpaceEditResult):void {if(!result.ok){this.message=result.error;this.render();return;}this.message="";this.v2SelectedSpace=result.selection;this.v2RightContext="space";setBackgroundSceneV2(result.scene,globalThis);}
  private renderSpaceInspector(scene:BackgroundSceneV2):HTMLElement {const selection=this.v2SelectedSpace;const space=selection&&findSpace(scene,selection);const panel=el("section","cm-v2-inspector cm-space-inspector");panel.dataset.inspector="space";if(!selection||!space){this.v2SelectedSpace=null;panel.textContent="Select a Space";return panel;}panel.append(this.inspectorHeading("SPACE",selection.id),this.row("TYPE",document.createTextNode(selection.kind[0].toUpperCase()+selection.kind.slice(1))),this.row("ID",Object.assign(el("div","cm-space-readonly"),{textContent:selection.id})));
    if(selection.kind==="marker"&&"position" in space)panel.append(this.row("X",this.numericStepper({value:space.position,displayValue:Number(formatWorldX(space.position)),step:1,onCommit:value=>this.applySpaceEdit(updateLogicMarker(scene,selection.id,{position:value}))})));
    else if(selection.kind==="range"&&"start" in space)panel.append(this.row("START",this.numericStepper({value:space.start,displayValue:Number(formatWorldX(space.start)),step:1,onCommit:value=>this.applySpaceEdit(updateLogicRange(scene,selection.id,{start:value}))})),this.row("END",this.numericStepper({value:space.end,displayValue:Number(formatWorldX(space.end)),step:1,onCommit:value=>this.applySpaceEdit(updateLogicRange(scene,selection.id,{end:value}))})),this.row("LENGTH",document.createTextNode(formatWorldX(space.end-space.start))));
    else if(selection.kind==="zone"&&"minX" in space)panel.append(this.row("MIN X",this.numericStepper({value:space.minX,displayValue:Number(formatWorldX(space.minX)),step:1,onCommit:value=>this.applySpaceEdit(updateLogicZone(scene,selection.id,{minX:value}))})),this.row("MAX X",this.numericStepper({value:space.maxX,displayValue:Number(formatWorldX(space.maxX)),step:1,onCommit:value=>this.applySpaceEdit(updateLogicZone(scene,selection.id,{maxX:value}))})),this.row("MIN Y",this.numericStepper({value:space.minY,step:1,onCommit:value=>this.applySpaceEdit(updateLogicZone(scene,selection.id,{minY:value}))})),this.row("MAX Y",this.numericStepper({value:space.maxY,step:1,onCommit:value=>this.applySpaceEdit(updateLogicZone(scene,selection.id,{maxY:value}))})),this.row("WIDTH",document.createTextNode(formatWorldX(space.maxX-space.minX))),this.row("HEIGHT",document.createTextNode(String(space.maxY-space.minY))));
    const remove=button("Delete Space",()=>this.applySpaceDelete(scene,selection));remove.className="cm-space-delete";panel.append(remove);if(this.message){const message=el("div","cm-pixel-msg");message.textContent=this.message;panel.append(message);}return panel;}
  private applySpaceDelete(scene:BackgroundSceneV2,selection:V2SpaceSelection):void {const result=deleteSpace(scene,selection);if(!result.ok){this.message=result.error;this.render();return;}this.message="";this.v2SelectedSpace=null;this.v2RightContext="scene-asset";setBackgroundSceneV2(result.scene,globalThis);}
  ...
  private syncOverlay(): void {
    ...
    if(v2Scene&&this.v2RightContext==="space"&&this.v2SelectedSpace?.kind==="zone"){this.syncZoneOverlay(v2Scene,this.v2SelectedSpace.id);return;}
  ...
  private syncZoneOverlay(scene:BackgroundSceneV2,zoneId:string):void {const zone=scene.sceneLogic?.spaces.zones.find(item=>item.id===zoneId);const canvas=document.querySelector("canvas#game") as HTMLCanvasElement|null;if(!this.visible||!zone||!canvas){this.removeOverlay();return;}const viewport=resolveCanvasViewportRect(canvas.getBoundingClientRect(),this.logicW,this.logicH);if(!viewport){this.removeOverlay();return;}const rect=zoneToCanvasRect(zone,this.currentScroll()),sx=viewport.width/this.logicW,sy=viewport.height/this.logicH;const overlay=this.ensureOverlay();overlay.style.left=`${viewport.left}px`;overlay.style.top=`${viewport.top}px`;overlay.style.width=`${viewport.width}px`;overlay.style.height=`${viewport.height}px`;overlay.style.pointerEvents="none";overlay.innerHTML="";const box=el("div","cm-bgr-placement-box cm-space-zone");box.dataset.zoneId=zone.id;box.style.left=`${rect.x*sx}px`;box.style.top=`${rect.y*sy}px`;box.style.width=`${Math.max(2,rect.width*sx)}px`;box.style.height=`${Math.max(2,rect.height*sy)}px`;box.style.pointerEvents="auto";box.style.cursor="grab";overlay.append(box);const label=el("div","cm-bgr-placement-label");label.textContent=`Scene Logic Zone · ${zone.id}`;overlay.append(label);}
  ...
  private onPointerDown(e: PointerEvent): void { if(!(e.target instanceof HTMLElement)||!e.target.closest(".cm-bgr-placement-box"))return;const p=this.pointerInternal(e);if(!p)return;const scene=getBackgroundSceneV2(globalThis);if(scene&&this.v2RightContext==="space"&&this.v2SelectedSpace?.kind==="zone"&&!this.v2PlacementTarget){const zone=scene.sceneLogic?.spaces.zones.find(item=>item.id===this.v2SelectedSpace!.id);if(!zone)return;e.preventDefault();e.stopPropagation();this.overlay?.setPointerCapture?.(e.pointerId);this.zoneDrag={pointerId:e.pointerId,zoneId:zone.id,anchor:{x:p.x-zone.minX,y:p.y-zone.minY},baseline:scene};return;}if(scene&&this.v2PlacementTarget){const track=findV2Track(scene,this.v2SelectedTrackId);const segment=this.v2PlacementTarget==="segment"?findV2Segment(scene,track?.id??"",this.v2SelectedSegmentId):null;const object=this.v2PlacementTarget==="object"?findV2Object(scene,track?.id??"",this.v2SelectedObjectId):null;if(!track||(!segment&&!object)||segment?.locked||object?.locked)return;e.preventDefault();e.stopPropagation();this.overlay?.setPointerCapture?.(e.pointerId);const point=segment?{x:segment.startTrackX,y:segment.offsetY}:{x:object!.startTrackX,y:object!.y};const origin=v2TrackPointToScreen(point,this.currentScroll(),track.parallax);this.drag={pointerId:e.pointerId,anchor:{x:p.x-origin.x,y:p.y-origin.y}};return;}const layer=this.editableSprite();if(!layer)return;e.preventDefault(); this.overlay?.setPointerCapture?.(e.pointerId); const owner=this.owner.kind==="chunk" ? {kind:"chunk" as const, chunkStartX:this.selectedChunkStart()} : {kind:"global" as const}; const origin=layerRenderedOrigin(layer,owner,this.currentScroll()); this.drag={pointerId:e.pointerId,anchor:{x:p.x-origin.x,y:p.y-origin.y}}; }
  private onPointerMove(e: PointerEvent): void { if(this.zoneDrag?.pointerId!==e.pointerId&&this.drag?.pointerId!==e.pointerId)return;const p=this.pointerInternal(e);if(!p)return;const scene=getBackgroundSceneV2(globalThis);if(this.zoneDrag&&scene){if(this.zoneDrag.pointerId!==e.pointerId)return;const zone=this.zoneDrag.baseline.sceneLogic?.spaces.zones.find(item=>item.id===this.zoneDrag!.zoneId);if(!zone)return;e.preventDefault();e.stopPropagation();this.applySpaceEdit(updateLogicZone(this.zoneDrag.baseline,zone.id,moveZone(zone,{x:p.x-this.zoneDrag.anchor.x-zone.minX,y:p.y-this.zoneDrag.anchor.y-zone.minY})));return;}const drag=this.drag;if(!drag)return;if(scene&&this.v2PlacementTarget){const track=findV2Track(scene,this.v2SelectedTrackId);if(!track)return;e.preventDefault();e.stopPropagation();const rendered={x:p.x-drag.anchor.x,y:p.y-drag.anchor.y};const authored=screenPointToV2TrackPoint(rendered,this.currentScroll(),track.parallax);if(this.v2PlacementTarget==="segment")this.applyV2Edit(updateV2Segment(scene,track.id,this.v2SelectedSegmentId,{startTrackX:authored.x,offsetY:authored.y}));else this.applyV2ObjectEdit(moveV2Object(scene,track.id,this.v2SelectedObjectId,authored.x,authored.y));return;}const layer=this.editableSprite();if(!layer)return;e.preventDefault(); const rendered={x:p.x-drag.anchor.x,y:p.y-drag.anchor.y}; const owner=this.owner.kind==="chunk" ? {kind:"chunk" as const, chunkStartX:this.selectedChunkStart()} : {kind:"global" as const}; const offset=renderedOriginToAuthoredOffset(rendered,layer,owner,this.currentScroll()); this.setDraft(updateSelectedSpriteOffset(this.draft,this.owner,this.selectedLayerId,offset,this.pixelSafe?"integer":"fractional"),false); }
  private onPointerUp(e: PointerEvent): void { if(this.zoneDrag?.pointerId===e.pointerId){e.preventDefault();e.stopPropagation();this.zoneDrag=null;this.syncOverlay();return;}if(this.drag?.pointerId!==e.pointerId) return;e.preventDefault();e.stopPropagation();this.endDrag();if(!getBackgroundSceneV2(globalThis))saveDraft(localStorage,this.draft);this.syncOverlay(); }
