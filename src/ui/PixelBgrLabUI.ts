...
export class PixelBgrLabUI {
  ...
  close(): void { this.visualPlacement = false; this.endTimelineDrag(); this.endCursorDrag(); this.endDrag(); this.removeOverlay(); if (!this.visible) { clearBackgroundPreviewState(globalThis); return; } this.visible = false; clearBackgroundPreviewState(globalThis); this.root.style.display = "none"; this.notifyOpenChange(); }
  show(): void { this.open(); }
  ...
  private gameplayX(): number { return Number((globalThis as any).__CM?.game?.world?.scrollX ?? 0); }
  private currentX(): number { const st=getBackgroundPreviewState(globalThis); return st.enabled ? st.scrollX : this.gameplayX(); }
  private setCurrentX(x: number, previewMode = true): void { if(previewMode) setBackgroundPreviewState({enabled:true,paused:true,scrollX:x},globalThis); }
  private renderPreview(): HTMLElement { const p=el("div","cm-pixel-toolbar cm-pixel-preview"); const st=getBackgroundPreviewState(globalThis); const currentX=this.currentX(); const jumps=chunkJumpState(this.draft.chunks,currentX); const mode=el("span","cm-mode-pill"); mode.textContent=st.enabled?"PREVIEW":"GAMEPLAY"; const label=el("span","cm-current-x"); label.textContent=`Current X ${Math.round(currentX)}`; const previewToggle=button(`Preview mode: ${st.enabled?"On":"Off"}`,()=>{setBackgroundPreviewState({enabled:!st.enabled,paused:true,scrollX:currentX},globalThis);this.render();}); const playPause=this.iconButton(st.paused?"Play":"Pause",st.paused?Play:Pause,st.paused?"Play":"Pause",()=>{setBackgroundPreviewState({enabled:true,paused:!st.paused,scrollX:currentX},globalThis);this.render();}); p.append(mode,label,previewToggle,this.iconButton("Previous chunk",SkipBack,"SkipBack",()=>{ if(jumps.previousX!==null){ this.setCurrentX(jumps.previousX,true); this.render(); }},!jumps.canPrevious),playPause,this.iconButton("Stop and return to start",Square,"Square",()=>{ const start=sceneTimelineBounds(this.draft.chunks,0).startX; setBackgroundPreviewState({enabled:true,paused:true,scrollX:start},globalThis); this.render(); }),this.iconButton("Next chunk",SkipForward,"SkipForward",()=>{ if(jumps.nextX!==null){ this.setCurrentX(jumps.nextX,true); this.render(); }},!jumps.canNext),this.row("speed",this.numericStepper({value:st.speed,step:10,onCommit:v=>setBackgroundPreviewState({speed:v},globalThis)}))); return p; }
  ...
  private currentScroll(): Point { return { x: this.currentX(), y: 0 }; }
