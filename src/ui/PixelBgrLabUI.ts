...
export class PixelBgrLabUI {
  ...
  private v2YRailDrag: (V2YRailDrag & { kind:"segment"|"object"; trackId:string; itemId:string; baseline:BackgroundSceneV2; captureTarget:HTMLElement|null }) | null = null;
  ...
  private renderOwned(): void {
    ...
    if (v2Scene) {
      ...
      const contextualEvent=this.renderV2EventSurface(v2Scene);if(contextualEvent)this.root.appendChild(contextualEvent);
      const contextualY=this.renderV2ContextualYSurface();if(contextualY){this.root.appendChild(contextualY);if(!this.v2YRailDrag)this.workspace.left.appendChild(this.renderV2YRail());}
      this.root.append(this.renderPreview([],projection.bounds));
      this.syncOverlay();
      return;
    }
  ...
  private renderV2YRail():HTMLElement {const rail=el("div","cm-v2-y-rail");rail.dataset.yRail="selection";rail.setAttribute("role","slider");rail.setAttribute("aria-label","Relative vertical position scrubber");rail.appendChild(el("span","cm-v2-y-rail-thumb"));rail.onpointerdown=e=>{const segment=this.selectedV2Segment();const object=segment?null:this.selectedV2Object();if(!segment&&!object)return;isolateTimelinePointerEvent(e);this.endV2YRailDrag();const rect=this.workspace.viewport.getBoundingClientRect();const captureTarget=e.currentTarget instanceof HTMLElement?e.currentTarget:null;captureTarget?.setPointerCapture?.(e.pointerId);this.v2YRailDrag={pointerId:e.pointerId,startClientY:e.clientY,originalY:segment?.segment.offsetY??object!.object.y,logicPerClientPx:rect.height>0?this.logicH/rect.height:1,kind:segment?"segment":"object",trackId:this.v2SelectedTrackId,itemId:segment?.segment.id??object!.object.id,baseline:(segment??object)!.scene,captureTarget};window.addEventListener("pointermove",this.onV2YRailPointerMove);window.addEventListener("pointerup",this.onV2YRailPointerUp);window.addEventListener("pointercancel",this.onV2YRailPointerUp);};return rail;}
  private onV2YRailPointerMove=(e:PointerEvent):void=>{const drag=this.v2YRailDrag;if(!drag||drag.pointerId!==e.pointerId)return;isolateTimelinePointerEvent(e);const y=v2YRailValue(drag,e.clientY);if(drag.kind==="segment")this.applyV2Edit(updateV2Segment(drag.baseline,drag.trackId,drag.itemId,{offsetY:y}));else this.applyV2ObjectEdit(updateV2Object(drag.baseline,drag.trackId,drag.itemId,{y}));};
