...
import type { BackgroundSceneV2, BackgroundSegment } from "../render/bg/v2/BackgroundV2Types";
...
export class PixelBgrLabUI {
  ...
  private renderV2Toolbar(): HTMLElement {
    ...
    const badge=el("span","cm-mode-pill"); badge.textContent="V2 · SEGMENT AUTHORING";
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    panel.append("V2 sequence segment authoring · objects, environment, gameplay reference, and track structure remain read-only");
    ...
    scroll.appendChild(timeline); panel.append(scroll,this.renderV2SegmentInspector(),this.renderPreview([],projection.bounds)); return panel;
  }
  ...
  private renderV2SegmentInspector(): HTMLElement {
    ...
    if(!selected||!track){p.append("Select a segment. Sequence tracks support M6 editing; repeat tracks are read-only.",this.parallaxPolicyInfo());return p;}
  ...
  private onPointerDown(e: PointerEvent): void { const layer=this.editableSprite(); const p=this.pointerInternal(e); if(!layer||!p) return; e.preventDefault(); this.overlay?.setPointerCapture?.(e.pointerId); const owner=this.owner.kind==="chunk" ? {kind:"chunk" as const, chunkStartX:this.selectedChunkStart()} : {kind:"global" as const}; const origin=layerRenderedOrigin(layer,owner,this.currentScroll()); this.drag={pointerId:e.pointerId,anchor:{x:p.x-origin.x,y:p.y-origin.y}}; }
