...
type V2RightContext = "scene-asset" | "space";
...
export class PixelBgrLabUI {
  ...
  private readonly sceneContentsExpanded = new Set<string>(["SEG", "OBJ", "EVE", "SPACE"]);
  ...
  private renderOwned(): void {
    ...
    if (v2Scene) {
      ...
      if (this.getActiveDevLab() === "scene") this.workspace.right.appendChild(this.v2RightContext==="space" ? this.renderSpaceInspector(v2Scene) : this.renderSceneAssetContext());
  ...
  private renderSpaceInspector(scene:BackgroundSceneV2):HTMLElement {const selection=this.v2SelectedSpace;const space=selection&&findSpace(scene,selection);const panel=el("section","cm-v2-inspector cm-space-inspector");panel.dataset.inspector="space";if(!selection||!space){this.v2SelectedSpace=null;panel.textContent="Select a Space";return panel;}panel.append(this.inspectorHeading("SPACE",selection.id),this.row("TYPE",document.createTextNode(selection.kind[0].toUpperCase()+selection.kind.slice(1))),this.row("ID",Object.assign(el("div","cm-space-readonly"),{textContent:selection.id})));
    if(selection.kind==="marker"&&"position" in space)panel.append(this.row("X",this.numericStepper({value:space.position,step:1,onCommit:value=>this.applySpaceEdit(updateLogicMarker(scene,selection.id,{position:value}))})));
    else if(selection.kind==="range"&&"start" in space)panel.append(this.row("START",this.numericStepper({value:space.start,step:1,onCommit:value=>this.applySpaceEdit(updateLogicRange(scene,selection.id,{start:value}))})),this.row("END",this.numericStepper({value:space.end,step:1,onCommit:value=>this.applySpaceEdit(updateLogicRange(scene,selection.id,{end:value}))})),this.row("LENGTH",document.createTextNode(String(space.end-space.start))));
    else if(selection.kind==="zone"&&"minX" in space)panel.append(this.row("MIN X",this.numericStepper({value:space.minX,step:1,onCommit:value=>this.applySpaceEdit(updateLogicZone(scene,selection.id,{minX:value}))})),this.row("MAX X",this.numericStepper({value:space.maxX,step:1,onCommit:value=>this.applySpaceEdit(updateLogicZone(scene,selection.id,{maxX:value}))})),this.row("MIN Y",this.numericStepper({value:space.minY,step:1,onCommit:value=>this.applySpaceEdit(updateLogicZone(scene,selection.id,{minY:value}))})),this.row("MAX Y",this.numericStepper({value:space.maxY,step:1,onCommit:value=>this.applySpaceEdit(updateLogicZone(scene,selection.id,{maxY:value}))})),this.row("WIDTH",document.createTextNode(String(space.maxX-space.minX))),this.row("HEIGHT",document.createTextNode(String(space.maxY-space.minY))));
