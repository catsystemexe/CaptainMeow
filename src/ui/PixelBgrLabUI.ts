...
import { Copy, Download, Eye, EyeOff, FolderOpen, Minus, Pause, Play, Plus, RotateCcw, Save, Trash2, Upload, X } from "lucide";
...
export class PixelBgrLabUI {
  ...
  setDisplayMode(mode: PixelBgrDisplayMode): void {
    ...
    if (mode === "game") {
      ...
      this.endTimelineDrag(); this.endV2SegmentDrag(); this.endV2EventDrag(); this.endV2YRailDrag(); this.endCursorDrag(); this.endDrag(); this.removeOverlay();
  ...
  private renderOwned(): void {
    ...
    if (v2Scene) {
      ...
      const headerBlock=el("div","cm-v2-left-block cm-v2-header-block");headerBlock.append(titlebar,this.renderV2Toolbar());
      const sourceBlock=el("div","cm-v2-left-block cm-v2-source-block");sourceBlock.append(this.renderV2Environment(v2Scene));
      this.root.append(headerBlock,sourceBlock);
      const inspectorStack=el("div","cm-v2-left-block cm-v2-selection-block cm-v2-inspector-stack");
      inspectorStack.appendChild(this.renderV2EventSurface(v2Scene));
      inspectorStack.append(this.renderV2ReservedInspector("TRIGGER"),this.renderV2ReservedInspector("MARKER"));this.root.appendChild(inspectorStack);
      const transportBlock=el("div","cm-v2-left-block cm-v2-transport-block");transportBlock.append(this.renderPreview([],projection.bounds));this.root.appendChild(transportBlock);
      this.syncOverlay();
      return;
    }
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    projection.events.forEach((event,index)=>{const selected=this.v2SelectedEventId===event.id;const marker=el("span",`cm-v2-event ${event.type}${event.enabled?"":" disabled"}${selected?" sel":""}`);marker.style.left=`${worldToTimelinePx(event.worldX,scale)}px`;marker.textContent=String(index+1);marker.title=event.label;marker.setAttribute("aria-label",`Event ${index+1}: ${event.label}, ${event.type}, X ${event.worldX}`);marker.onpointerdown=e=>{if(e.button===2){isolateTimelinePointerEvent(e);this.v2SelectedEventId=event.id;return;}this.beginV2EventDrag(e,event.id,scale);};marker.oncontextmenu=e=>this.openV2EventContextMenu(e,event.id);eventRow.appendChild(marker);});timeline.appendChild(eventRow);
    ...
    scroll.appendChild(timeline);if(this.selectedV2Segment()||this.selectedV2Object()){panel.classList.add("has-y-rail");panel.append(this.renderV2YRail(),this.renderV2ContextualYSurface());}panel.appendChild(scroll); return panel;
  }
  ...
  private fitV2Timeline(baseWidthPx:number):void {this.v2TimelineZoom=timelineFitZoom(this.workspace.timeline.clientWidth,baseWidthPx);this.render();const scroll=this.workspace.timeline.querySelector<HTMLElement>(".cm-v2-timeline-scroll");if(scroll)scroll.scrollLeft=0;}
  ...
  private openV2EntityContextMenu(e:MouseEvent,label:string,actions:readonly [string,()=>void][]):void {e.preventDefault();e.stopPropagation();this.closeEntityContextMenu?.();const menu=el("div","cm-v2-entity-menu");menu.setAttribute("role","menu");menu.setAttribute("aria-label",`${label} actions`);const close=()=>{menu.remove();document.removeEventListener("pointerdown",outside);document.removeEventListener("keydown",keydown);if(this.closeEntityContextMenu===close)this.closeEntityContextMenu=null;};const outside=(event:PointerEvent)=>{if(event.target instanceof Node&&!menu.contains(event.target))close();};const keydown=(event:KeyboardEvent)=>{if(event.key==="Escape"){event.preventDefault();close();}};for(const [name,action] of actions){const item=button(name,()=>{close();action();});item.setAttribute("role","menuitem");menu.appendChild(item);}menu.style.left=`${e.clientX}px`;menu.style.top=`${e.clientY}px`;document.body.appendChild(menu);this.closeEntityContextMenu=close;document.addEventListener("pointerdown",outside);document.addEventListener("keydown",keydown);menu.querySelector("button")?.focus();}
  private openV2VisualContextMenu(e:MouseEvent,trackId:string,kind:"segment"|"object",itemId:string):void {isolateTimelinePointerEvent(e);kind==="segment"?this.selectV2Segment(trackId,itemId,false):this.selectV2Object(trackId,itemId,false);const scene=getBackgroundSceneV2(globalThis);if(!scene)return;const actions:readonly [string,()=>void][]=kind==="segment"?[["Duplicate",()=>this.applyV2Edit(duplicateV2Segment(scene,trackId,itemId))],["Delete",()=>this.applyV2Edit(deleteV2Segment(scene,trackId,itemId))]]:[["Duplicate",()=>this.applyV2ObjectEdit(duplicateV2Object(scene,trackId,itemId))],["Delete",()=>this.applyV2ObjectEdit(deleteV2Object(scene,trackId,itemId))]];this.openV2EntityContextMenu(e,kind,actions);}
  private openV2EventContextMenu(e:MouseEvent,eventId:string):void {isolateTimelinePointerEvent(e);this.endV2EventDrag();this.v2SelectedEventId=eventId;const scene=getBackgroundSceneV2(globalThis);const event=scene?.events?.find(item=>item.id===eventId);if(!scene||!event)return;const actions:[string,()=>void][]=[];if(event.type==="signal")actions.push(["Duplicate",()=>this.applyV2EventEdit(duplicateV2SceneEvent(scene,eventId))]);actions.push([event.enabled?"Disable":"Enable",()=>this.applyV2EventEdit(updateV2SceneEvent(scene,eventId,{enabled:!event.enabled}))],["Delete",()=>this.applyV2EventEdit(deleteV2SceneEvent(scene,eventId))]);this.openV2EntityContextMenu(e,"Event",actions);}
  private applyV2EventEdit(result:V2SceneEventEditResult):void {if(!result.ok){this.message=result.error;this.render();return;}this.message="";this.v2SelectedEventId=result.eventId;setBackgroundSceneV2(result.scene,globalThis);}
  private beginV2EventDrag(e:PointerEvent,eventId:string,scale:TimelineScale):void {isolateTimelinePointerEvent(e);this.endV2EventDrag();const baseline=getBackgroundSceneV2(globalThis);if(!baseline)return;this.v2SelectedEventId=eventId;const captureTarget=e.currentTarget instanceof HTMLElement?e.currentTarget:null;captureTarget?.setPointerCapture?.(e.pointerId);this.v2EventDrag={pointerId:e.pointerId,eventId,startClientX:e.clientX,scale,baseline,captureTarget,active:true};window.addEventListener("pointermove",this.onV2EventPointerMove);window.addEventListener("pointerup",this.onV2EventPointerUp);window.addEventListener("pointercancel",this.onV2EventPointerUp);}
  ...
  private renderV2EventSurface(scene:BackgroundSceneV2):HTMLElement {
    ...
    orderedV2SceneEvents(scene).forEach((event,index)=>{const name=event.type==="signal"?event.name:event.id;const row=button("",()=>{this.v2SelectedEventId=event.id;this.render();});row.className=`cm-v2-logic-row${event.id===this.v2SelectedEventId?" sel":""}`;row.setAttribute("role","option");row.setAttribute("aria-selected",String(event.id===this.v2SelectedEventId));row.setAttribute("aria-label",`Event ${index+1}: ${name}, ${event.type}`);row.title=name;const ordinal=el("span","cm-v2-logic-number");ordinal.textContent=`${index+1}.`;const eventName=el("span","cm-v2-logic-name");eventName.textContent=name;const type=el("span","cm-v2-logic-type");type.textContent=event.type;row.append(ordinal,eventName,type);if(event.type==="signal")eventName.ondblclick=e=>{e.stopPropagation();const input=el("input","cm-v2-logic-name-edit");input.value=event.name;input.setAttribute("aria-label",`Edit Event ${index+1} name`);const commit=()=>{const value=input.value.trim();if(value&&value!==event.name)this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{name:value}));else this.render();};input.onblur=commit;input.onkeydown=key=>{if(key.key==="Enter")input.blur();if(key.key==="Escape"){input.value=event.name;input.blur();}};eventName.replaceWith(input);input.focus();input.select();};list.appendChild(row);});
    if(!list.childElementCount){const empty=el("span","cm-v2-logic-empty");empty.textContent="none";list.appendChild(empty);}surface.appendChild(list);return surface;
  }
  private plainV2InspectorHeader(label:string):HTMLElement {const header=el("div","cm-v2-inspector-header");header.textContent=label;return header;}
  private renderV2ReservedInspector(label:"TRIGGER"|"MARKER"):HTMLElement {const surface=el("section","cm-v2-inspector-section cm-v2-inspector-placeholder");surface.setAttribute("aria-disabled","true");surface.title="Reserved for future logic authoring";surface.append(this.plainV2InspectorHeader(label),document.createTextNode("reserved"));return surface;}
  private renderV2ContextualYSurface():HTMLElement {
    ...
    if(segment){
      const track=findV2Track(segment.scene,this.v2SelectedTrackId)!;
      row.appendChild(this.numericStepper({value:segment.segment.offsetY,displayValue:Math.round(segment.segment.offsetY),step:1,onCommit:value=>this.applyV2Edit(updateV2Segment(segment.scene,track.id,segment.segment.id,{offsetY:value}))}));
    }else{
      const track=findV2Track(object!.scene,this.v2SelectedTrackId)!;
      row.appendChild(this.numericStepper({value:object!.object.y,displayValue:Math.round(object!.object.y),step:1,onCommit:value=>this.applyV2ObjectEdit(updateV2Object(object!.scene,track.id,object!.object.id,{y:value}))}));
    }
    surface.title=id;surface.append(row);return surface;
  }
  private renderV2YRail():HTMLElement {const rail=el("div","cm-v2-y-rail");rail.dataset.yRail="selection";rail.setAttribute("role","slider");rail.setAttribute("aria-label","Relative vertical position scrubber");const thumb=el("span","cm-v2-y-rail-thumb");rail.appendChild(thumb);rail.onpointerdown=e=>{const segment=this.selectedV2Segment();const object=segment?null:this.selectedV2Object();if(!segment&&!object)return;isolateTimelinePointerEvent(e);this.endV2YRailDrag();const rect=this.workspace.viewport.getBoundingClientRect();const captureTarget=e.currentTarget instanceof HTMLElement?e.currentTarget:null;captureTarget?.setPointerCapture?.(e.pointerId);this.v2YRailDrag={pointerId:e.pointerId,startClientY:e.clientY,originalY:segment?.segment.offsetY??object!.object.y,logicPerClientPx:rect.height>0?this.logicH/rect.height:1,kind:segment?"segment":"object",trackId:this.v2SelectedTrackId,itemId:segment?.segment.id??object!.object.id,baseline:(segment??object)!.scene,captureTarget,thumb};window.addEventListener("pointermove",this.onV2YRailPointerMove);window.addEventListener("pointerup",this.onV2YRailPointerUp);window.addEventListener("pointercancel",this.onV2YRailPointerUp);};return rail;}
  private onV2YRailPointerMove=(e:PointerEvent):void=>{const drag=this.v2YRailDrag;if(!drag||drag.pointerId!==e.pointerId)return;isolateTimelinePointerEvent(e);drag.thumb?.style.setProperty("transform",`translateY(${e.clientY-drag.startClientY}px)`);const y=v2YRailValue(drag,e.clientY);if(drag.kind==="segment")this.applyV2Edit(updateV2Segment(drag.baseline,drag.trackId,drag.itemId,{offsetY:y}));else this.applyV2ObjectEdit(updateV2Object(drag.baseline,drag.trackId,drag.itemId,{y}));};
  ...
  private endV2YRailDrag():void {const drag=this.v2YRailDrag;if(!drag)return;drag.captureTarget?.releasePointerCapture?.(drag.pointerId);this.v2YRailDrag=null;window.removeEventListener("pointermove",this.onV2YRailPointerMove);window.removeEventListener("pointerup",this.onV2YRailPointerUp);window.removeEventListener("pointercancel",this.onV2YRailPointerUp);}
