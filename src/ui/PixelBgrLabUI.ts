...
import { ChevronDown, ChevronRight, Copy, Download, Eye, EyeOff, FolderOpen, Minus, Pause, Play, Plus, Power, RotateCcw, Save, Trash2, Upload, X } from "lucide";
...
export class PixelBgrLabUI {
  ...
  private sceneContentsVisualChild(kind:"segment"|"object",trackId:string,id:string,enabled:boolean):HTMLElement {const selected=this.v2SelectedTrackId===trackId&&(kind==="segment"?this.v2SelectedSegmentId===id:this.v2SelectedObjectId===id);const row=el("div",`cm-scene-tree-child${selected?" sel":""}`);const toggle=()=>{const scene=getBackgroundSceneV2(globalThis);if(!scene)return;if(kind==="segment"){const result=updateV2Segment(scene,trackId,id,{enabled:!enabled});if(result.ok)setBackgroundSceneV2(result.scene,globalThis);}else{const result=updateV2Object(scene,trackId,id,{enabled:!enabled});if(result.ok)setBackgroundSceneV2(result.scene,globalThis);}};const eye=this.iconButton(`${enabled?"Hide":"Show"} ${kind} ${id}`,enabled?Eye:EyeOff,enabled?"Eye":"EyeOff",toggle);eye.className="cm-scene-tree-action";eye.onclick=e=>{e.stopPropagation();toggle();};const select=button(id,()=>kind==="segment"?this.selectV2Segment(trackId,id):this.selectV2Object(trackId,id));select.className="cm-scene-tree-child-label";row.append(eye,select,document.createTextNode(trackId));return row;}
  private sceneContentsEventChild(scene:BackgroundSceneV2,event:NonNullable<BackgroundSceneV2["events"]>[number],index:number):HTMLElement {const name=event.type==="signal"?event.name:event.id;const row=el("div",`cm-scene-tree-child${event.id===this.v2SelectedEventId?" sel":""}`);const toggle=()=>{const result=updateV2SceneEvent(scene,event.id,{enabled:!event.enabled});if(result.ok)setBackgroundSceneV2(result.scene,globalThis);};const power=this.iconButton(`${event.enabled?"Disable":"Enable"} Event ${name}`,Power,"Power",toggle);power.className="cm-scene-tree-action";power.dataset.action="activation";power.dataset.state=event.enabled?"all":"none";power.onclick=e=>{e.stopPropagation();toggle();};const label=button(`${index+1}. ${name}`,()=>this.selectV2Event(event.id));label.className="cm-scene-tree-child-label";const type=el("span","cm-v2-logic-type");type.textContent=event.type;row.append(power,label,type);if(event.type==="signal"){const input=el("input","cm-v2-logic-name-edit");input.value=event.name;input.setAttribute("aria-label",`Edit Event ${index+1} name`);input.onclick=e=>{e.stopPropagation();this.selectV2Event(event.id,false);};input.onchange=()=>{const value=input.value.trim();if(value&&value!==event.name)this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{name:value}));};row.replaceChild(input,label);}return row;}
