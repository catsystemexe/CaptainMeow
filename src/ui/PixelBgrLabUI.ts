...
import { Download, FolderOpen, Pause, Play, RotateCcw, Save, Square, Trash2, Upload, X } from "lucide";
...
export class PixelBgrLabUI {
  ...
  private renderOwned(): void {
    ...
    if (v2Scene) {
      this.root.append(this.renderV2Environment(v2Scene));
  ...
  private renderV2Toolbar(): HTMLElement {
    ...
    actions.append(
      ...
      this.iconButton("Load saved scene",FolderOpen,"FolderOpen",()=>this.loadV2()),
      this.iconButton("Clear saved scene",Trash2,"Trash2",()=>this.clearSavedV2()),
    ...
    const opacityRow=el("label","cm-scene-opacity-row"); opacityRow.append("Opacity",opacity);
    p.append(heading,actions,opacityRow);
    return p;
  }
