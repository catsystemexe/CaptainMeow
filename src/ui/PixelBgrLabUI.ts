...
export class PixelBgrLabUI {
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    for(const lane of projection.lanes){
        ...
        const add=button("+",()=>this.openV2LaneInsertMenu(add,row,lane));add.className="cm-v2-lane-add";add.title=`Add to ${lane.label}`;add.setAttribute("aria-label",`Add to ${lane.label}`);add.onpointerdown=isolateTimelinePointerEvent;row.appendChild(add);
  ...
  private openV2LaneInsertMenu(trigger:HTMLElement,row:HTMLElement,lane:V2TimelineProjection["lanes"][number]):void {
    this.closeLaneInsertMenu?.();
    ...
    const segment=button("Segment",()=>insert("segment"));segment.setAttribute("role","menuitem");const object=button("Object",()=>insert("object"));object.setAttribute("role","menuitem");menu.append(segment,object);row.appendChild(menu);this.closeLaneInsertMenu=close;document.addEventListener("pointerdown",outside);document.addEventListener("keydown",keydown);segment.focus();
  }
