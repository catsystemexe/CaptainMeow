...
export class PixelBgrLabUI {
  ...
  private cursorDrag: { pointerId: number; scale: TimelineScale; timelineLeft: number; captureTarget: HTMLElement | null; active: boolean } | null = null;
  ...
  private renderTimelineSection(): HTMLElement {
    ...
    timeline.onclick=(e)=>{ if (this.timelineDrag || this.cursorDrag) return; if (e.target !== timeline && !(e.target as HTMLElement).classList.contains("cm-ruler")) return; const rect=timeline.getBoundingClientRect(); const x=Math.round(timelinePxToWorld(e.clientX-rect.left, scale)); setBackgroundPreviewState({enabled:true,paused:true,scrollX:x},globalThis); this.render(); };
    ...
    const cursor=el("div","cm-cursor"); cursor.style.left=`${worldToTimelinePx(preview.scrollX,scale)/10}%`; cursor.title=`Drag preview cursor ${preview.scrollX}`; cursor.onpointerdown=(e)=>this.beginCursorDrag(e, timeline, scale); timeline.appendChild(cursor);
  ...
  private beginCursorDrag(e: PointerEvent, timeline: HTMLElement, scale: TimelineScale): void {
    ...
    captureTarget.setPointerCapture?.(e.pointerId);
    this.cursorDrag={pointerId:e.pointerId,scale,timelineLeft:timeline.getBoundingClientRect().left,captureTarget,active:true};
  ...
  private updateCursorDrag(e: PointerEvent): void {
    ...
    const x=Math.round(timelinePxToWorld(e.clientX-drag.timelineLeft, drag.scale));
    setBackgroundPreviewState({enabled:true,paused:true,scrollX:x},globalThis);
  }
