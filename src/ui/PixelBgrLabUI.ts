...
import { Copy, Download, Eye, EyeOff, FolderOpen, Maximize2, Minus, Pause, Play, Plus, RotateCcw, Save, Trash2, Upload, X } from "lucide";
...
export function fitV2TimelineZoom(authoredSpan: number, viewportWidth: number): typeof PIXEL_BGR_TIMELINE_ZOOM_LEVELS[number] {
  const target = Math.max(1, viewportWidth) / Math.max(1, authoredSpan);
  return [...PIXEL_BGR_TIMELINE_ZOOM_LEVELS].reverse().find(level => level <= target) ?? PIXEL_BGR_TIMELINE_ZOOM_LEVELS[0];
}
...
export class PixelBgrLabUI {
  ...
  private renderV2Toolbar(): HTMLElement {
    ...
    const saved=loadBackgroundSceneV2(localStorage).ok;
    const remove=this.iconButton("Delete scene",Trash2,"Trash2",()=>{this.deleteConfirmationOpen=true;this.render();},!saved);remove.classList.add("cm-scene-delete");actions.append(remove);
    if(this.sceneMenuOpen) actions.appendChild(this.renderSceneMenu());
    if(this.deleteConfirmationOpen) actions.appendChild(this.renderDeleteConfirmation());
  ...
  private renderSceneMenu():HTMLElement {const menu=el("div","cm-scene-menu");menu.setAttribute("role","menu");menu.setAttribute("aria-label","Available scenes");for(const entry of SCENE_LAB_SCENE_CATALOG){const item=button(entry.label,()=>this.selectScene(entry));item.setAttribute("role","menuitem");menu.appendChild(item);}const saved=loadBackgroundSceneV2(localStorage);if(saved.ok){const item=button(`Saved: ${saved.scene.id}`,()=>{this.closeSceneMenu(false);this.loadV2();});item.setAttribute("role","menuitem");menu.appendChild(item);}for(const [label,action] of [["Import JSON...",()=>this.importV2File()],["Export JSON...",()=>this.exportV2File()]] as const){const item=button(label,()=>{this.closeSceneMenu(false);action();});item.setAttribute("role","menuitem");menu.appendChild(item);}return menu;}
  private renderDeleteConfirmation():HTMLElement {const popover=el("div","cm-scene-delete-confirm");popover.setAttribute("role","alertdialog");popover.setAttribute("aria-label","Delete saved scene?");const title=el("strong");title.textContent="Delete saved scene?";const detail=el("span");detail.textContent="This removes the saved scene data. This action cannot be undone.";const controls=el("div");controls.append(button("Cancel",()=>{this.deleteConfirmationOpen=false;this.render();}),button("Delete",()=>this.clearSavedV2()));popover.append(title,detail,controls);return popover;}
  private clearSavedV2():void {if(!loadBackgroundSceneV2(localStorage).ok){this.deleteConfirmationOpen=false;this.message="No saved scene to delete";this.render();return;}clearBackgroundSceneV2(localStorage);this.deleteConfirmationOpen=false;this.message="Deleted saved scene data (active scene unchanged)";this.render();}
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    const zoomFit=this.iconButton("Fit authored range",Maximize2,"Maximize2",()=>this.fitV2Timeline(projection));zoomFit.className="cm-v2-zoom-button";
    ...
    zoomControls.append(zoomOut,zoomFit,zoomIn);gutter.appendChild(zoomControls);
  ...
  private fitV2Timeline(projection:V2TimelineProjection):void {const points:number[]=[];for(const lane of projection.lanes)for(const track of lane.tracks){for(const segment of track.segments)points.push(segment.startX,segment.endX);for(const object of track.objects)points.push(object.x,object.width===null?object.x:object.x+object.width);}const start=points.length?Math.min(...points):0;const end=points.length?Math.max(...points):0;this.v2TimelineZoom=fitV2TimelineZoom(end-start,this.workspace.viewport.clientWidth);this.render();}
  ...
  private renderPreview(chunks=this.draft.chunks, _explicitBounds?: {startX:number;endX:number}): HTMLElement { const p=el("div","cm-pixel-toolbar cm-pixel-preview cm-scene-transport"); const paused=this.gameplayPaused(); const start=0; const playPause=this.iconButton(paused?"Play":"Pause",paused?Play:Pause,paused?"Play":"Pause",()=>{ (globalThis as any).__CM?.loop?.setPaused?.(!paused); this.render(); }); p.append(this.iconButton("Reset to scene start",RotateCcw,"RotateCcw",()=>{this.setCurrentX(start,true);this.render();}),playPause); return p; }
