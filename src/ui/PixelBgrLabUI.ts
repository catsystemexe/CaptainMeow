...
export class PixelBgrLabUI {
  ...
  private renderOwned(): void {
    ...
    if (v2Scene) {
      ...
      this.root.append(headerBlock,this.v2Spacer(),sourceBlock,this.v2Spacer());
  ...
  private renderV2Environment(scene: BackgroundSceneV2): HTMLElement {
    ...
    row.append("env: starfield", check(Boolean(starfield),enabled=>this.applyV2EnvironmentEdit(enabled?enableV2Starfield(scene):disableV2Starfield(scene))));
