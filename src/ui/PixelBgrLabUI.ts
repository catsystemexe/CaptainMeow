...
export class PixelBgrLabUI {
  ...
  constructor() {
    ...
    this.workspace.leftCanvas.appendChild(this.root);
  ...
  private renderOwned(): void {
    ...
    if (v2Scene) {
      ...
      inspectorStack.append(this.renderV2ReservedInspector("TRIGGER"),this.renderV2ReservedInspector("MARKER"));upperContent.appendChild(inspectorStack);
  ...
  private renderV2Environment(scene: BackgroundSceneV2): HTMLElement {
    ...
    backdropRow.append(backdropEye,`BGR: ${backdrop?.asset.id??"unavailable"}`);
    const row=el("div","cm-scene-environment-row");
    const environmentEye=this.iconButton(starfield?"Hide environment":"Show environment",starfield?Eye:EyeOff,starfield?"Eye":"EyeOff",()=>this.applyV2EnvironmentEdit(starfield?disableV2Starfield(scene):enableV2Starfield(scene)));
    environmentEye.className="cm-v2-eye";environmentEye.setAttribute("aria-pressed",starfield?"true":"false");
    row.append(environmentEye,"ENV: starfield");
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    scroll.appendChild(timeline);const contextualY=this.renderV2ContextualYSurface();if(contextualY.dataset.contextKind)panel.dataset.contextKind=contextualY.dataset.contextKind;panel.append(contextualY,this.renderV2YRail(),scroll);return panel;
  }
  ...
  private renderV2EventSurface(scene:BackgroundSceneV2):HTMLElement {
    ...
    orderedV2SceneEvents(scene).forEach((event,index)=>{const name=event.type==="signal"?event.name:event.id;const row=el("div",`cm-v2-logic-row${event.id===this.v2SelectedEventId?" sel":""}`);row.setAttribute("role","option");row.setAttribute("aria-selected",String(event.id===this.v2SelectedEventId));row.setAttribute("aria-label",`Event ${index+1}: ${name}, ${event.type}`);row.title=name;row.onclick=()=>{this.selectV2Event(event.id);};const ordinal=el("span","cm-v2-logic-number");ordinal.textContent=`${index+1}.`;const type=el("span","cm-v2-logic-type");type.textContent=event.type;if(event.type==="signal"){const input=el("input","cm-v2-logic-name-edit");input.value=event.name;input.setAttribute("aria-label",`Edit Event ${index+1} name`);input.onclick=e=>{e.stopPropagation();this.selectV2Event(event.id,false);};input.onpointerdown=e=>e.stopPropagation();input.onchange=()=>{const value=input.value.trim();if(value&&value!==event.name)this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{name:value}));else input.value=event.name;};input.onkeydown=key=>{if(key.key==="Enter")input.blur();if(key.key==="Escape"){input.value=event.name;input.blur();}};row.append(ordinal,input,type);}else{const eventName=el("span","cm-v2-logic-name");eventName.textContent=name;row.append(ordinal,eventName,type);}list.appendChild(row);});
  ...
  private renderPreview(chunks=this.draft.chunks, explicitBounds?: {startX:number;endX:number}, primary=false): HTMLElement { const p=el("div","cm-pixel-toolbar cm-pixel-preview cm-scene-transport"); const paused=this.gameplayPaused(); const start=explicitBounds?.startX??sceneTimelineBounds(chunks,0).startX; const iconSize=primary?60:16; const playPause=this.iconButton(paused?"Play":"Pause",paused?Play:Pause,paused?"Play":"Pause",()=>{ (globalThis as any).__CM?.loop?.setPaused?.(!paused); this.render(); },false,iconSize); p.append(this.iconButton("Reset to scene start",RotateCcw,"RotateCcw",()=>{(globalThis as any).__CM?.loop?.setPaused?.(true);this.setCurrentX(start,true);this.render();},false,iconSize),playPause); return p; }
