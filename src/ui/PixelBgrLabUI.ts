...
import { createV2SceneEvent, deleteV2SceneEvent, updateV2SceneEvent, type V2SceneEventEditResult } from "./PixelBgrV2SceneEvents";
...
export class PixelBgrLabUI {
  ...
  private renderOwned(): void {
    ...
    if(!this.v2YRailDrag)this.workspace.left.querySelector(".cm-v2-y-rail")?.remove();
    ...
    if (v2Scene) {
      ...
      const contextualY=this.renderV2ContextualYSurface();inspectorStack.appendChild(contextualY);if(contextualY.dataset.contextualY&&!this.v2YRailDrag)this.workspace.left.appendChild(this.renderV2YRail());
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    for(const event of projection.events){const selected=this.v2SelectedEventId===event.id;const marker=el("span",`cm-v2-event ${event.type}${event.enabled?"":" disabled"}${selected?" sel":""}`);marker.style.left=`${worldToTimelinePx(event.worldX,scale)}px`;marker.textContent=event.type==="level-end"?"▼ END":`◇ ${event.label}`;marker.title=`${event.type} @ ${event.worldX}`;marker.onpointerdown=e=>this.beginV2EventDrag(e,event.id,scale);eventRow.appendChild(marker);}timeline.appendChild(eventRow);
  ...
  private renderV2InspectorHeader(label:string,enabled:boolean,onToggle:()=>void,onDelete:()=>void):HTMLElement {const header=el("div","cm-v2-inspector-header");const actions=el("span","cm-v2-inspector-actions");const eye=this.iconButton(enabled?`Disable ${label}`:`Enable ${label}`,enabled?Eye:EyeOff,enabled?"Eye":"EyeOff",onToggle);eye.className="cm-v2-eye";eye.setAttribute("aria-pressed",String(enabled));const remove=this.iconButton(`Delete ${label}`,Trash2,"Trash2",onDelete);remove.className="cm-v2-eye";actions.append(eye,remove);header.append(label,actions);return header;}
  private renderV2EventSurface(scene:BackgroundSceneV2):HTMLElement {const event=scene.events?.find(item=>item.id===this.v2SelectedEventId);const surface=el("section","cm-v2-inspector-section");surface.dataset.inspectorSection="event";if(!event){surface.classList.add("cm-v2-inspector-placeholder");surface.append(this.plainV2InspectorHeader("EVENT"),document.createTextNode("none"));return surface;}const nameRow=this.row("name",event.type==="signal"?text(event.name,name=>this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{name}))):document.createTextNode(event.id));nameRow.classList.add("cm-v2-inspector-name-row");const detailRow=el("div","cm-v2-inspector-detail-row");detailRow.append(this.row("type",document.createTextNode(event.type)),this.row("X",this.numericStepper({value:event.worldX,step:16,min:0,onCommit:worldX=>this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{worldX}))})));surface.append(this.renderV2InspectorHeader("EVENT",event.enabled,()=>this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{enabled:!event.enabled})),()=>this.applyV2EventEdit(deleteV2SceneEvent(scene,event.id))),nameRow,detailRow);return surface;}
  ...
  private renderV2ContextualYSurface():HTMLElement {
    ...
    if(!segment&&!object){const surface=el("section","cm-v2-inspector-section cm-v2-inspector-placeholder");surface.dataset.inspectorSection="layer";surface.append(this.plainV2InspectorHeader("LAYER"),document.createTextNode("none"));return surface;}
    const surface=el("section","cm-v2-selected-y cm-v2-inspector-section");surface.dataset.contextualY="selection";surface.dataset.inspectorSection="layer";
    const id=segment?.segment.id??object!.object.id;
    const enabled=segment?.segment.enabled??object!.object.enabled;
    const title=this.renderV2InspectorHeader("LAYER",enabled,()=>{if(segment)this.applyV2Edit(updateV2Segment(segment.scene,this.v2SelectedTrackId,segment.segment.id,{enabled:!enabled}));else this.applyV2ObjectEdit(updateV2Object(object!.scene,this.v2SelectedTrackId,object!.object.id,{enabled:!enabled}));},()=>{this.v2PlacementTarget=null;if(segment)this.applyV2Edit(deleteV2Segment(segment.scene,this.v2SelectedTrackId,segment.segment.id));else this.applyV2ObjectEdit(deleteV2Object(object!.scene,this.v2SelectedTrackId,object!.object.id));});
    ...
    surface.append(title,this.row("ID",document.createTextNode(id)),row);return surface;
  }
