...
export const PIXEL_BGR_TIMELINE_ZOOM_LEVELS = [0.05, 0.1, 0.2, 0.5, 1, 2] as const;
...
export class PixelBgrLabUI {
  ...
  constructor() {
    ...
    style.textContent += `.cm-scene-action-row{position:relative}.cm-scene-menu{position:absolute;z-index:4;top:100%;left:0;min-width:142px;padding:2px;background:#071521;border:1px solid #28516d;box-shadow:0 4px 10px rgba(0,0,0,.45)}.cm-scene-menu button{display:block;width:100%;padding:4px 6px;text-align:left;white-space:nowrap}.cm-scene-menu button:hover,.cm-scene-menu button:focus-visible{background:#163b52;color:#fff}.cm-v2-workspace{flex:1 1 auto;min-height:0;overflow:visible}.cm-v2-inspector{min-width:0;padding:5px 2px;margin:0;border:0;border-radius:0;overflow:visible}.cm-v2-inspector-title{margin:0 0 5px;color:#8ee8ff;font-size:11px;letter-spacing:.04em}.cm-v2-panel{--cm-v2-gutter-width:100px;flex:1 0 auto;width:calc(100% + var(--cm-v2-gutter-width));min-width:0;overflow:hidden;display:grid;grid-template-columns:var(--cm-v2-gutter-width) minmax(0,1fr);padding:2px 0;margin:0 0 0 calc(-1 * var(--cm-v2-gutter-width));border:0;border-radius:0;box-sizing:border-box}.cm-v2-timeline-gutter{width:100px;height:128px;display:grid;grid-template-rows:20px repeat(4,27px);overflow:hidden;box-sizing:border-box;background:#000}.cm-v2-zoom-controls{display:flex;align-items:center;justify-content:center;gap:4px}.cm-v2-zoom-button,.cm-v2-eye{display:inline-grid;place-items:center;width:18px;min-width:18px;height:18px;min-height:0!important;padding:0!important;color:#ddd}.cm-v2-zoom-button svg,.cm-v2-eye svg{width:13px;height:13px}.cm-v2-eye[data-visibility="mixed"]{opacity:.55}.cm-v2-eye[data-visibility="none"]{opacity:.45}.cm-v2-gutter-row{height:27px;display:flex;align-items:center;gap:2px;min-width:0;color:#ddd;font-size:10px;white-space:nowrap}.cm-v2-role-label{overflow:hidden;text-overflow:ellipsis}.cm-v2-parallax{width:31px!important;min-width:31px!important;height:18px!important;min-height:18px!important;padding:0 1px!important;border:0!important;background:#000!important;text-align:right;-moz-appearance:textfield}.cm-v2-parallax::-webkit-inner-spin-button,.cm-v2-parallax::-webkit-outer-spin-button{-webkit-appearance:none;margin:0}.cm-v2-timeline-scroll{width:100%;max-width:100%;height:145px;overflow-x:auto;overflow-y:hidden;overscroll-behavior-x:contain;scrollbar-gutter:stable;box-sizing:border-box}.cm-bgr-workspace-timeline .cm-v2-timeline-scroll{flex:0 0 auto;min-height:0}.cm-v2-timeline{position:relative;height:128px;background:rgba(3,12,22,.78);user-select:none;pointer-events:auto}.cm-v2-ruler{height:20px;border:0;pointer-events:auto}.cm-v2-ruler .cm-ruler-tick{height:14px;font-size:9px;color:#bbb;border-color:rgba(255,255,255,.22);padding-left:2px}.cm-v2-lane{position:absolute;left:0;right:0;box-sizing:border-box;overflow:hidden}.cm-v2-lane-label{display:inline-flex;align-items:center;gap:2px;min-width:0;height:23px;color:#ddd;font-size:10px;white-space:nowrap;pointer-events:auto}.cm-v2-lane-track-button{overflow:hidden;text-overflow:ellipsis}.cm-v2-lane-track-select{flex:1 1 auto;width:38px}.cm-v2-lane-track-button{min-width:0;height:19px;padding:0;border:0;background:transparent;color:inherit;font:inherit;text-align:left;cursor:pointer}.cm-v2-lane-track-select{min-width:0;max-width:164px;height:19px;padding:0 2px;border:1px solid rgba(255,255,255,.28);border-radius:2px;background:#111;color:#ddd;font:9px ui-monospace,Menlo,Consolas,monospace}.cm-v2-overlap{position:absolute;top:5px;height:16px;background:repeating-linear-gradient(135deg,rgba(255,75,90,.7),rgba(255,75,90,.7) 4px,rgba(255,75,90,.25) 4px,rgba(255,75,90,.25) 8px);pointer-events:none;z-index:1}.cm-v2-segment,.cm-v2-object{position:absolute;top:4px;height:17px;box-sizing:border-box;border:1px solid #52d7ff;border-radius:2px;background:rgba(45,132,180,.78);color:#fff;font-size:9px;line-height:13px;padding:1px 3px;overflow:hidden;text-overflow:clip;white-space:nowrap;pointer-events:none;z-index:2}.cm-v2-segment.editable{pointer-events:auto;cursor:grab}.cm-v2-segment.sel,.cm-v2-object.sel{border-color:#ffe66d;box-shadow:0 0 0 1px rgba(255,230,109,.45)}.cm-v2-segment-handle{position:absolute;top:0;bottom:0;width:6px;background:rgba(255,255,255,.2)}.cm-v2-segment-handle.left{left:0;cursor:ew-resize}.cm-v2-segment-handle.right{right:0;cursor:ew-resize}.cm-v2-object{top:6px;height:13px;min-width:8px;border-color:#c084fc;background:rgba(126,72,170,.9);line-height:9px}.cm-v2-segment.disabled,.cm-v2-object.disabled{opacity:.35;filter:saturate(.3)}.cm-v2-foreground{background:rgba(255,209,102,.025)}.cm-v2-viewport-range{position:absolute;top:0;bottom:0;box-sizing:border-box;border-left:1px solid rgba(255,255,255,.65);border-right:1px solid rgba(255,255,255,.65);background:rgba(255,255,255,.055);pointer-events:none;z-index:6}.cm-v2-cursor{top:0;bottom:0;border-left:2px solid #fff;filter:drop-shadow(0 0 2px rgba(0,0,0,.95));z-index:8}.cm-v2-cursor::after{top:0;border-top:0;border-bottom:8px solid #fff}.cm-scene-compact-section{margin:2px 0;border:0;padding:0}.cm-scene-section-title{display:none}.cm-scene-environment-row{display:flex;align-items:center;gap:4px;white-space:nowrap}.cm-scene-environment-row input[type=number]{width:48px;min-height:22px}.cm-scene-action-row{display:flex;align-items:center;gap:2px;flex-wrap:nowrap;margin:3px 0}.cm-scene-action-row .cm-transport-button{flex:1 1 0;width:auto;min-width:0;min-height:22px}.cm-scene-transport{flex-wrap:nowrap;margin:3px 0}`;
  ...
  private renderOwned(): void {
    ...
    if (v2Scene) {
      ...
      this.root.append(this.renderV2Toolbar(),this.renderV2Environment(v2Scene),this.renderLeftTools(),this.renderPreview([],projection.bounds));
      this.syncOverlay();
      return;
    }
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    for(const lane of projection.lanes){
      ...
      if(lane.tracks.length===1){const track=lane.tracks[0];const selectTrack=button(lane.label,()=>this.selectV2Track(track.id));selectTrack.className="cm-v2-lane-track-button cm-v2-role-label";selectTrack.title=`Select track ${track.id}`;label.appendChild(selectTrack);}
      else if(lane.tracks.length>1){const role=el("span","cm-v2-role-label");role.textContent=lane.label;label.append(role);const trackSelect=el("select","cm-v2-lane-track-select");trackSelect.setAttribute("aria-label",`Select ${lane.label} track`);for(const track of lane.tracks){const option=el("option");option.value=track.id;option.textContent=track.label;trackSelect.appendChild(option);}const selectedTrack=lane.tracks.find(track=>track.id===this.v2SelectedTrackId);trackSelect.value=selectedTrack?.id??lane.tracks[0].id;trackSelect.onpointerdown=isolateTimelinePointerEvent;trackSelect.onchange=()=>this.selectV2Track(trackSelect.value);label.appendChild(trackSelect);}
      ...
      gutterRow.append(label,eye,parallax);gutter.appendChild(gutterRow);
    }
    ...
    const viewport=el("div","cm-v2-viewport-range");viewport.style.left=`${worldToTimelinePx(viewportRange.startX,scale)}px`;viewport.style.width=`${worldToTimelinePx(viewportRange.endX,scale)-worldToTimelinePx(viewportRange.startX,scale)}px`;viewport.title=`Visible viewport ${Math.round(viewportRange.startX)}..${Math.round(viewportRange.endX)}`;timeline.appendChild(viewport);
    ...
    scroll.appendChild(timeline); panel.append(gutter,scroll); return panel;
  }
  private updateV2TimelineIndicators():void {const scale=this.v2RenderedTimelineScale;if(!scale)return;const viewport=this.workspace.timeline.querySelector<HTMLElement>(".cm-v2-viewport-range");const cursor=this.workspace.timeline.querySelector<HTMLElement>(".cm-v2-cursor");if(viewport){const range=timelineViewportRange(this.currentScroll().x,this.logicW);viewport.style.left=`${worldToTimelinePx(range.startX,scale)}px`;viewport.style.width=`${worldToTimelinePx(range.endX,scale)-worldToTimelinePx(range.startX,scale)}px`;viewport.title=`Visible viewport ${Math.round(range.startX)}..${Math.round(range.endX)}`;}if(cursor){const playerX=this.currentX();cursor.style.left=`${worldToTimelinePx(playerX,scale)}px`;cursor.title=`Drag Player X cursor ${Math.round(playerX)}`;}}
  ...
  private changeV2TimelineZoom(direction:-1|1):void {const currentIndex=PIXEL_BGR_TIMELINE_ZOOM_LEVELS.indexOf(this.v2TimelineZoom as typeof PIXEL_BGR_TIMELINE_ZOOM_LEVELS[number]);const nextIndex=Math.max(0,Math.min(PIXEL_BGR_TIMELINE_ZOOM_LEVELS.length-1,currentIndex+direction));const next=PIXEL_BGR_TIMELINE_ZOOM_LEVELS[nextIndex];if(next===this.v2TimelineZoom)return;const oldScroll=this.workspace.timeline.querySelector<HTMLElement>(".cm-v2-timeline-scroll");const oldCursor=this.workspace.timeline.querySelector<HTMLElement>(".cm-v2-cursor");const cursorViewportX=oldScroll&&oldCursor?oldCursor.offsetLeft-oldScroll.scrollLeft:null;this.v2TimelineZoom=next;this.render();if(cursorViewportX!==null){const newScroll=this.workspace.timeline.querySelector<HTMLElement>(".cm-v2-timeline-scroll");const newCursor=this.workspace.timeline.querySelector<HTMLElement>(".cm-v2-cursor");if(newScroll&&newCursor)newScroll.scrollLeft=Math.max(0,newCursor.offsetLeft-cursorViewportX);}}
