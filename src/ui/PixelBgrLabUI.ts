...
export class PixelBgrLabUI {
  ...
  private renderOwned(): void {
    ...
    if (v2Scene) {
      ...
      const contextualY=this.renderV2ContextualYSurface();if(contextualY){inspectorStack.appendChild(contextualY);if(!this.v2YRailDrag)this.workspace.left.appendChild(this.renderV2YRail());}
      const contextualEvent=this.renderV2EventSurface(v2Scene);if(contextualEvent)inspectorStack.appendChild(contextualEvent);
  ...
  private renderV2EventSurface(scene:BackgroundSceneV2):HTMLElement|null {const event=scene.events?.find(item=>item.id===this.v2SelectedEventId);if(!event)return null;const surface=el("section","cm-v2-inspector-section");surface.dataset.inspectorSection="event";surface.append(this.renderV2InspectorHeader("EVENT",event.enabled,()=>this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{enabled:!event.enabled})),()=>this.applyV2EventEdit(deleteV2SceneEvent(scene,event.id))),this.row("name",event.type==="signal"?text(event.name,name=>this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{name}))):document.createTextNode(event.id)),this.row("type",document.createTextNode(event.type)),this.row("X",this.numericStepper({value:event.worldX,step:16,min:0,onCommit:worldX=>this.applyV2EventEdit(updateV2SceneEvent(scene,event.id,{worldX}))})));return surface;}
  private renderV2ReservedInspector(label:"TRIGGER"|"MARKER"):HTMLElement {const surface=el("section","cm-v2-inspector-section cm-v2-inspector-placeholder");surface.setAttribute("aria-disabled","true");surface.title="Reserved for future logic authoring";const header=el("div","cm-v2-inspector-header");header.textContent=label;surface.appendChild(header);return surface;}
