...
export function fitV2TimelineZoom(authoredBounds:{startX:number;endX:number},viewportWidth:number):typeof PIXEL_BGR_TIMELINE_ZOOM_LEVELS[number] {
  const maximum=Math.max(0,viewportWidth)/Math.max(1,authoredBounds.endX-authoredBounds.startX);
  return [...PIXEL_BGR_TIMELINE_ZOOM_LEVELS].reverse().find(level=>level<=maximum)??PIXEL_BGR_TIMELINE_ZOOM_LEVELS[0];
}
...
export class PixelBgrLabUI {
  ...
  private renderV2Toolbar(): HTMLElement {
    ...
    const remove=this.iconButton("Delete scene",Trash2,"Trash2",()=>{this.deleteConfirmationOpen=true;this.render();},!loadBackgroundSceneV2(localStorage).ok);
    remove.className="cm-transport-button cm-scene-delete";
  ...
  private renderDeleteConfirmation():HTMLElement {const popover=el("div","cm-scene-delete-confirm");popover.setAttribute("role","alertdialog");popover.setAttribute("aria-label","Delete saved scene?");const title=el("strong");title.textContent="Delete saved scene?";const detail=el("span");detail.textContent="This removes the saved scene data.";const warning=el("span");warning.textContent="This action cannot be undone.";const controls=el("div");controls.append(button("Cancel",()=>{this.deleteConfirmationOpen=false;this.render();}),button("Delete",()=>this.clearSavedV2()));popover.append(title,detail,warning,controls);return popover;}
  private clearSavedV2():void {clearBackgroundSceneV2(localStorage);this.deleteConfirmationOpen=false;this.message="Saved scene deleted (active scene unchanged)";this.render();}
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    const fit=this.iconButton("Fit authored range",Maximize2,"Fit",()=>this.fitV2Timeline(projection));fit.className="cm-v2-zoom-button";
  ...
  private fitV2Timeline(projection:V2TimelineProjection):void {
    const points:number[]=[];
    for(const lane of projection.lanes)for(const track of lane.tracks){for(const segment of track.segments)points.push(segment.startX,segment.endX);for(const object of track.objects)points.push(object.x,object.width===null?object.x:object.x+object.width);}
    const bounds=points.length?{startX:Math.min(...points),endX:Math.max(...points)}:{startX:0,endX:0};
    const viewportWidth=this.workspace.timeline.querySelector<HTMLElement>(".cm-v2-timeline-scroll")?.clientWidth??this.workspace.viewport.clientWidth;
    this.v2TimelineZoom=fitV2TimelineZoom(bounds,viewportWidth);
    this.render();
  }
  ...
  private renderPreview(chunks=this.draft.chunks, explicitBounds?: {startX:number;endX:number}): HTMLElement { const p=el("div","cm-pixel-toolbar cm-pixel-preview cm-scene-transport"); const paused=this.gameplayPaused(); const start=getBackgroundSceneV2(globalThis)?0:explicitBounds?.startX??sceneTimelineBounds(chunks,0).startX; const playPause=this.iconButton(paused?"Play":"Pause",paused?Play:Pause,paused?"Play":"Pause",()=>{ (globalThis as any).__CM?.loop?.setPaused?.(!paused); this.render(); }); p.append(this.iconButton("Reset to scene start",RotateCcw,"RotateCcw",()=>{this.setCurrentX(start,true);this.render();}),playPause); return p; }
