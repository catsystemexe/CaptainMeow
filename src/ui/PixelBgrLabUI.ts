...
export class PixelBgrLabUI {
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    const status=el("div","cm-v2-timeline-status");
    status.textContent=`V2 Timeline · Player X ${Math.round(projection.playerX)} · authored ${projection.bounds.startX}..${projection.bounds.endX}`;
    panel.append(status);
    ...
    for(const lane of projection.lanes){
      ...
      rows.forEach((track,index)=>{
        ...
        if(track){
          const trackOnlySelected=this.v2SelectedTrackId===track.id&&!this.v2SelectedSegmentId&&!this.v2SelectedObjectId;
          const identity=el("span",`cm-v2-track-label${trackOnlySelected?" sel":""}`); identity.textContent=`${track.id} · ${track.mode}${track.enabled?"":" · disabled"}`; identity.onpointerdown=e=>{isolateTimelinePointerEvent(e);this.selectV2Track(track.id);}; row.appendChild(identity);
