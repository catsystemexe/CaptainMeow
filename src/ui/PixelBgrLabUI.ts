...
export class PixelBgrLabUI {
  ...
  private renderV2Toolbar(): HTMLElement {
    ...
    actions.append(
      this.iconButton("Load current scene",FolderOpen,"FolderOpen",()=>this.loadV2()),
  ...
  private renderV2Timeline(projection: V2TimelineProjection): HTMLElement {
    ...
    const baseWidthPx=Math.max(1000,Math.ceil(contentSpan));
    const scale=createExactTimelineScale(projection.bounds.startX,projection.bounds.endX,baseWidthPx,this.v2TimelineZoom);
  ...
  private renderSceneToolbar(): HTMLElement {
    ...
    p.append(
      this.iconButton("Load current scene",FolderOpen,"FolderOpen",()=>this.setDraft(getBackgroundScene(globalThis)??this.draft)),
