...
const scene: BackgroundSceneV2 = { version:2,id:"objects",environment:{starfield:{seed:2,density:.2}},tracks:[
  {id:"near",name:"Near",role:"near",mode:"sequence",enabled:true,parallax:{x:.7,y:.3},zBase:4,segments:[{id:"untouched-segment",startTrackX:5,widthPx:20,asset:{id:"stars",url:"/stars.png"},offsetY:8,opacity:1,blend:"normal",localZ:0,enabled:true}],objects:[{id:"existing",asset:{id:"stars",url:"/stars.png"},startTrackX:10,y:20,width:30,height:40,localZ:2,opacity:.5,blend:"normal",enabled:true}]},
...
assert.deepEqual({trackId:moved.trackId,objectId:moved.objectId},{trackId:"front",objectId:copy.id},"canvas movement retains object selection identity");
assert.equal(moved.scene.tracks[0].segments[0],duplicated.scene.tracks[0].segments[0],"object canvas movement leaves segments untouched");
