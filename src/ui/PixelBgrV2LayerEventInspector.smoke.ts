...
assert.match(eventSurface,/cm-scene-tree-child/);assert.match(source,/grid-template-columns:22px minmax\(0,1fr\) max-content/);
assert.match(eventSurface,/aria-label.*Event/);assert.match(eventSurface,/cm-v2-logic-name-edit/,"signal names are directly editable in their numbered row");
