...
import { consumeBackgroundMarkerRuntimeReset, getBackgroundState } from "../BackgroundState";
...
export class WebGLSceneRenderer {
  ...
  render(alpha: number = 1): void {
    ...
    if (resetSerial !== this.seenMarkerResetSerial) {
      ...
      resetBackgroundMarkerRuntime(this.markerRuntime, null, levelX);
    ...
    let layers = scene
      ? composeBackgroundLayers(scene, resolveActiveBackgroundChunks(scene, levelX, 0, 0))
    ...
    if (scene && selectBackgroundFallback(backgroundState) === "layers") {
      ...
      for (const marker of evaluateBackgroundMarkerCrossings(this.markerRuntime, sceneKey, levelX, markers)) {
