...
const scene=createBackgroundV2VisualVerificationScene();assert(saveBackgroundSceneV2(storage,scene).ok);const stored=JSON.parse(data.get(PIXEL_BGR_V2_DRAFT_KEY)!);assert.deepEqual(stored.tracks[0].objects[0].asset,{id:"bgr-test-solid"});assert.deepEqual(stored.tracks[1].segments[0].asset,{id:"bgr-test-stripes"});const loaded=loadBackgroundSceneV2(storage);assert(loaded.ok);if(loaded.ok)assert.deepEqual(loaded.scene,scene);assert.equal(data.get("captain-meow.pixel-bgr.draft"),"v1");
