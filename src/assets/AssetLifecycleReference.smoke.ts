...
const v1 = findAssetReferences(assetId("b1_pixel_stars"), index);
...
assert.equal(findAssetReferences(assetId("bgr_demo_orientation"), index).length, 0);
assert(findUnusedAssetCandidates().some(({ assetId, label }) => assetId === "bgr_demo_orientation" && label === "UNUSED_CANDIDATE"));
assert.equal(assessAssetRemoval(assetId("desert_near_band")).safeAgainstKnownRepositoryReferences, false);
...
const assessment = assessAssetRemoval(assetId("desert_near_band"), ASSET_CATALOG, informational);
