...
assert.equal((source.match(/statesAndDiagnosticsDock\.appendChild\(runtimeDiagnosticsSection\)/g) ?? []).length, 1, "exactly one Runtime Diagnostics panel is mounted in the states diagnostics dock");
...
assert(source.indexOf('authoringSections.id = "ds-fsm-authoring-sections"') < source.indexOf('statesAndDiagnosticsDock.appendChild(runtimeDiagnosticsSection)'), "FSM LAB authoring owns the Runtime Diagnostics dock");
