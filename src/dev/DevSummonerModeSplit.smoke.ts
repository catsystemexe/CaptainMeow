...
assert.equal(panel.findById("ds-fsm-lab-section")!.children[2].id, "ds-fsm-runtime-diagnostics", "RUNTIME DIAGNOSTICS follows Basic Setup in FSM mode");
assert(panel.findById("ds-fsm-preset-section"), "FSM Presets section is present in FSM mode");
assert(panel.findById("ds-fsm-basic-setup"), "FSM Basic Setup section is present in FSM mode");
assert(panel.findById("ds-fsm-runtime-diagnostics"), "FSM Runtime Diagnostics section is present in FSM mode");
