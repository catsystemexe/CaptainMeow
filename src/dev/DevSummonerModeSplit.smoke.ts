...
assert.equal(panel.findById("ds-fsm-lab-section")!.children[0].id, "ds-fsm-group-setup", "global/preset setup is the first FSM level");
assert.equal(panel.findById("ds-fsm-group-setup")!.children[0].id, "ds-fsm-preset-section", "Preset is the first global setup row");
assert.equal(panel.findById("ds-fsm-group-setup")!.children[1].id, "ds-fsm-basic-setup", "remaining group setup follows Preset");
assert.equal(panel.findById("ds-fsm-lab-section")!.children[1].id, "ds-fsm-preset-editor", "state editing follows the complete group setup");
