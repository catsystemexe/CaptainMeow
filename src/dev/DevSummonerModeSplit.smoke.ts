...
assert(panel.findById("ds-fsm-lab-section")!.children.includes(panel.findById("ds-shared-spawn-section")!), "shared spawn block is owned by FSM Lab");
...
assert(panel.findById("ds-fsm-preset"), "Group mode does not hide/remove the FSM selector");
clickButtonByText(panel, "Single");
...
assert.equal(emitted.at(-1)?.type, EventType.SPAWN_ENEMY_GROUP, "FSM Group spawn emits group event");
...
assert.equal(panel.findAll((el) => el.textContent === "Count").length, 0, "No Count-based conversion label was added");
