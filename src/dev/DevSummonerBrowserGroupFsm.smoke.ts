...
function mountAndSpawnGroup(input: { enemyTypeId: string; fsmPresetId?: string }) {
  ...
  assert.equal(panel.findById("ds-fsm-runtime-diagnostics"), null, "FSM runtime diagnostics panel is hidden from normal browser path");
  assert.equal(panel.findAll((el) => el.id === "ds-fsm-runtime-diagnostics").length, 0, "browser path has no duplicate runtime diagnostics panels");
