...
assert.deepEqual(DEV_LAB_MODES, ["scene", "sequence", "enemy", "hud"], "the canonical mode set is exact");
assert.equal(host.getActive(), "scene", "Scene is the default Lab");
assert.deepEqual(DEV_LAB_MODES.map(mode => host.bodies[mode].hidden), [false, true, true, true]);
...
assert.deepEqual(DEV_LAB_MODES.map(mode => host.bodies[mode].hidden), [true, true, false, true]);
host.setActive("hud");
assert.deepEqual(DEV_LAB_MODES.map(mode => host.bodies[mode].hidden), [true, true, true, false]);
...
selector.children[2].listeners.get("click")?.();
