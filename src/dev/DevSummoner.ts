...
export function createDevSummonerPanelStyle(): string {
  return [
    ...
    "border-radius:0", "display:flex", "flex-direction:column", "gap:3px",
...
function applyLabelTextStyle(el: HTMLElement, prominence: "primary" | "secondary" = "primary"): void {
  ...
  el.style.color = prominence === "primary" ? "#aaa" : "#929292";
  el.style.opacity = "1";
...
function createSelectLabel(text: string, prominence: "primary" | "secondary" = "primary"): HTMLLabelElement {
  ...
  span.textContent = text.endsWith(":") ? text : `${text}:`;
...
export class DevSummoner {
  ...
  init(): void {
    ...
    fsmLabSection.style.cssText = "display:none;flex-direction:column;gap:3px;width:100%;max-width:100%;overflow:visible;";
    ...
    const makeParamStepper = (label: string, key: GroupParamKey, defaultValue: number) => {
      ...
      labelNode.textContent = label.endsWith(":") ? label : `${label}:`;
    ...
    const enemyMovement = makeMovementControls("ds", "Movement", "path:");
    ...
    const groupMovement = makeMovementControls("ds-group", "Move", "path:");
    ...
    fsmPresetSection.style.cssText = "display:grid;grid-template-columns:auto minmax(0,1fr);gap:2px;align-items:center;min-width:0;";
    const fsmPresetLabel = document.createElement("span");
    fsmPresetLabel.textContent = "preset:";
    applyLabelTextStyle(fsmPresetLabel);
    ...
    fsmBasicSection.style.cssText = "display:flex;flex-direction:column;gap:3px;";

    const fsmTypeRow = document.createElement("label");
    fsmTypeRow.id = "ds-fsm-type-count-row";
    fsmTypeRow.style.cssText = "display:grid;grid-template-columns:auto minmax(0,1fr) auto auto;gap:3px;align-items:center;min-width:0;";
    ...
    fsmFormationButtons.style.cssText = "display:grid;grid-template-columns:repeat(5,18px);gap:0;min-width:0;";
    const fsmCoherenceLabel = document.createElement("span");
    fsmCoherenceLabel.textContent = "coh:";
    applyLabelTextStyle(fsmCoherenceLabel);
    const fsmRigidButton = document.createElement("button");
    const fsmElasticButton = document.createElement("button");
    for (const [button, label] of [[fsmRigidButton, "Rigid"], [fsmElasticButton, "Elastic"]] as const) {
      button.type = "button";
      button.textContent = label;
      button.title = `${label} FSM group coherence`;
      button.setAttribute("aria-label", `${label} FSM group coherence`);
    }
    ...
    const refreshFsmFormationIcons = () => {
      for (const { id, button } of fsmFormationIconButtons) {
        ...
        button.setAttribute("aria-checked", String(active));
        styleTextChoiceButton(button, active);
      }
      const rigid = fsmElasticitySlider.value === 0;
      fsmRigidButton.setAttribute("aria-pressed", String(rigid));
      fsmElasticButton.setAttribute("aria-pressed", String(!rigid));
      styleTextChoiceButton(fsmRigidButton, rigid);
      styleTextChoiceButton(fsmElasticButton, !rigid);
    };
    fsmRigidButton.addEventListener("click", () => { fsmElasticitySlider.setValue(0); refreshFsmFormationIcons(); if (enemyLabMode === "fsm") editFsmBasicSetupDraft?.(); });
    ...
    presetPanel.style.cssText = "display:flex;flex-direction:column;gap:3px;padding:0;background:transparent;font:12px monospace;overflow:visible;";
    const presetList = document.createElement("select");
    applyTextSelectStyle(presetList);
    ...
    authoringSections.style.cssText = "display:flex;flex-direction:column;gap:3px;overflow:visible;";
    ...
    statesHeading.style.cssText = "display:flex;align-items:center;gap:2px;font-weight:bold;color:#fff;";
    const statesLabel = Object.assign(document.createElement("span"), { textContent: "states:" });
    applyLabelTextStyle(statesLabel);
    statesHeading.appendChild(statesLabel);
    const stateToolbar = document.createElement("div");
    stateToolbar.id = "ds-fsm-state-toolbar";
    stateToolbar.style.cssText = "display:flex;gap:1px;margin-left:auto;";
    const stateToolButton = (icon: string, label: string) => { const button = document.createElement("button"); button.type = "button"; button.textContent = icon; button.title = label; button.setAttribute("aria-label", label); applyIconButtonStyle(button); stateToolbar.appendChild(button); return button; };
    const addStateBtn = stateToolButton(ICONS.new, "Add state");
    const dupStateBtn = stateToolButton(ICONS.duplicate, "Duplicate selected state");
    const delStateBtn = stateToolButton(ICONS.delete, "Delete selected state");
    const upStateBtn = stateToolButton(ICONS.up, "Move selected state up");
    const downStateBtn = stateToolButton(ICONS.down, "Move selected state down");
    statesHeading.appendChild(stateToolbar);
    ...
    stateList.style.cssText = "display:flex;gap:2px;max-width:100%;overflow-x:auto;overflow-y:hidden;padding-bottom:1px;";
    ...
    const behaviorBlock = document.createElement("label"); behaviorBlock.setAttribute("data-fsm-editor-block", "Behavior"); behaviorBlock.style.cssText = "display:grid;grid-template-columns:auto minmax(0,1fr);gap:3px;align-items:center;";
    const behaviorLabel = Object.assign(document.createElement("span"), { textContent: "behav:" }); applyLabelTextStyle(behaviorLabel); behaviorBlock.appendChild(behaviorLabel);
    const movementPresetInput = document.createElement("select"); applyTextSelectStyle(movementPresetInput); for (const id of Object.keys(EnemyBehaviorPresets).sort()) appendOption(movementPresetInput, id, id); behaviorBlock.appendChild(movementPresetInput); editorSection.appendChild(behaviorBlock);
    const formationBlock = document.createElement("div"); formationBlock.setAttribute("data-fsm-editor-block", "Formation"); formationBlock.style.cssText = "display:flex;flex-direction:column;gap:3px;";
    ...
    const stateSpacingSlider = createRangeRow("ds-fsm-state-spacing", "spac:", { ...ENEMY_GROUP_PARAM_LIMITS.formation.spacing, step: 2 });
    const stateElasticitySlider = createRangeRow("ds-fsm-state-elasticity", "elast:", { min: 0, max: 10, default: 0, step: 1 });
    ...
    statesSection.appendChild(editorSection);
...
${d.states.join(", ")}` : "No preset selected";
      ...
      for (const row of rows) {
        ...
        stateButton.textContent = String(row.number);
        stateButton.title = `${row.behaviorPresetId || row.id} — ${row.triggerSummary}`;
        stateButton.setAttribute("aria-label", `Select state ${row.number}: ${row.behaviorPresetId || row.id}`);
        stateButton.setAttribute("aria-pressed", String(row.selected));
        stateButton.setAttribute("data-fsm-state-row", String(row.number));
        styleTextChoiceButton(stateButton, row.selected);
        stateButton.style.flex = "0 0 20px";
        stateButton.addEventListener("click", () => { authoringModel.selectState(row.id); expandedStateId = row.id; renderPresetEditor(); });
        const summary = document.createElement("span");
        summary.setAttribute("data-fsm-state-trigger-summary", row.id);
        summary.textContent = row.triggerSummary;
        stateTriggerSummaryNodes.set(row.id, summary);
      ...
      for (const { id, button } of stateShapeButtons) { const active = id === selectedFormationShape; styleTextChoiceButton(button, active, authoringReadOnly || !view); button.disabled = authoringReadOnly || !view; }
      ...
      styleTextChoiceButton(runtimeDiagnosticsToggle, runtimeDiagnosticsDockOpen);
      ...
      const selectedStateIndex = rows.findIndex((row) => row.selected);
      setIconButtonDisabled(addStateBtn, authoringReadOnly);
      setIconButtonDisabled(dupStateBtn, authoringReadOnly || selectedStateIndex < 0);
      setIconButtonDisabled(delStateBtn, authoringReadOnly || rows.length <= 1 || selectedStateIndex < 0);
      setIconButtonDisabled(upStateBtn, authoringReadOnly || selectedStateIndex <= 0);
      setIconButtonDisabled(downStateBtn, authoringReadOnly || selectedStateIndex < 0 || selectedStateIndex >= rows.length - 1);
    ...
    const reorderSelectedState = (offset: number) => { const id = authoringModel.draft?.selectedStateId; const states = authoringModel.draft?.preset.graph.states ?? []; const index = states.findIndex((state) => String(state.id) === id); if (id && index >= 0) { authoringModel.reorderState(id, index + offset); authoringModel.normalizeSequentialTriggers(); } renderPresetEditor(); };
    upStateBtn.addEventListener("click", () => reorderSelectedState(-1));
    downStateBtn.addEventListener("click", () => reorderSelectedState(1));
    ...
    const refreshEnemyLabMode = () => {
      ...
      if (isSimple) {
        ...
        applyTextSelectStyle(groupEnemySelect);
        if (!groupTypeRow.contains(countLabel)) groupTypeRow.appendChild(countLabel);
        ...
        if (!fsmTypeRow.contains(countLabel)) fsmTypeRow.appendChild(countLabel);
