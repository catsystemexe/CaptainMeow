...
export function renderFsmRuntimeDiagnosticsText(bundle: FsmRuntimeDiagnosticsBundle): string {
  ...
  const lines = [
    ...
    `Basic Speed (runtime) baseSpeed: ${fmt(primary.baseSpeed, 1)}`,
    `State Speed × speedMultiplier: ${fmt(primary.speedMultiplier, 2)}`,
    `Effective Speed effectiveSpeed: ${fmt(primary.effectiveSpeed, 1)}`,
