...
import { isPlayerRenderVisible } from "../../render/webgl/WebGLSceneRenderer";
...
function main(): void {
  ...
  assert.equal(isPlayerRenderVisible(visiblePlayer, 0.15), false);
  ...
  const playerRef = store.spawn((p: any) => Object.assign(p, {
    ...
    radius: 3, bodyRadius: 20, energy: 1, energyMax: 5, invulnT: 0, deadT: 0, pendingKill: false,
  }));
  const bus = makeBus();
  ...
  assert(player.invulnT < 2.25 && player.invulnT > 0, "immunity must continue through fly-in");
  playerSystem.update(0.4, actions());
